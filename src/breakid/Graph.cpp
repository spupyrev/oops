...
std::vector<std::shared_ptr<Permutation>> perms;
...
uint Graph::getNbNodesFromGraph() { return (uint)saucy_g->n; }

uint Graph::getNbEdgesFromGraph() { return (uint)saucy_g->e; }

uint Graph::getColorOf(uint node) { return saucy_g->colors[node]; }

uint Graph::nbNeighbours(uint node) { return saucy_g->adj[node + 1] - saucy_g->adj[node]; }

uint Graph::getNeighbour(uint node, uint nbthNeighbour) { return saucy_g->edg[saucy_g->adj[node] + nbthNeighbour]; }

void Graph::setNodeToNewColor(uint node) { saucy_g->colors[node] = colorcount.size(); }
...
static int addSaucyPermutation(int n, const int *ct_perm, int nsupp, int *, void *) {
  ...
  std::shared_ptr<Permutation> perm = std::make_shared<Permutation>();
  for (int i = 0; i < n; ++i) {
    if (i != ct_perm[i]) {
...
void Graph::getSymmetryGeneratorsInternal(std::vector<std::shared_ptr<Permutation>> &out_perms) {
...
Graph::Graph(std::unordered_set<std::shared_ptr<Clause>, UVecHash, UvecEqual> &clauses) {
  // TODO: Why are we making undirected graphs? Efficiency? In principle, a lot could be directed (e.g., only edge from
  // clause to lit -> more in LP)
...
Graph::Graph(std::unordered_set<std::shared_ptr<Rule>, UVecHash, UvecEqual> &rules) {
  ...
  // colorn for body nodes of rule-type 2 or 5 with bound, where n is obtained from bound through the boundToColor map
  // below. arrows between positive and negative literals of the same atoms arrows from head to literals occurring in it
  // (and back) arrows from body to literals occurring in it (posbodylits and negbodylits can be treated as one large
  // list; separation is only for printing purposes) (and back) arrows from head to body (and back)
...
Graph::Graph(std::unordered_set<std::shared_ptr<PBConstraint>, UVecHash, UvecEqual> &constrs) {
  ...
  auto getNewNode = [&neighbours]() -> uint {
  ...
  std::map<std::pair<uint, uint>, uint>
      litCoeff2node; // maps a lit and coefficient to a node (for node reuse and keeping graph small)
  auto getnode = [&node2color, this, &litCoeff2node, &getCoeffColor, &neighbours, &getNewNode](uint lit,
                                                                                               uint coeff) -> uint {
...
uint Graph::getNbNodes() { return getNbNodesFromGraph(); }

uint Graph::getNbEdges() { return getNbEdgesFromGraph(); }
...
void Graph::getSymmetryGenerators(std::vector<std::shared_ptr<Permutation>> &out_perms) {
  out_perms.clear();
