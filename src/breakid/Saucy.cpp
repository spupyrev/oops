...
static int do_find_min(struct coloring *c, int t) { return array_find_min(c->lab + t, c->clen[t] + 1) + t; }

static int find_min(struct saucy *s, int t) { return do_find_min(&s->right, t); }
...
static int is_undirected_automorphism(struct saucy *s) {
  ...
  for (i = 0; i < s->ndiffs; ++i) {
    j = s->unsupp[i];
    if (!check_mapping(s, s->adj, s->edg, j))
      return 0;
  }
  return 1;
}
...
static int is_directed_automorphism(struct saucy *s) {
  ...
  for (i = 0; i < s->ndiffs; ++i) {
    ...
    if (!check_mapping(s, s->adj, s->edg, j))
      return 0;
    if (!check_mapping(s, s->dadj, s->dedg, j))
      return 0;
  }
  return 1;
}
...
static int at_terminal(struct saucy *s) { return s->nsplits == s->n; }
...
static int is_a_pair(struct saucy *s, int k) { return s->unpairs[k] != -1; }
...
static int ref_singleton_directed(struct saucy *s, struct coloring *c, int cf) {
  return ref_singleton(s, c, s->adj, s->edg, cf) && ref_singleton(s, c, s->dadj, s->dedg, cf);
}
...
static int ref_nonsingle_directed(struct saucy *s, struct coloring *c, int cf) {
  return ref_nonsingle(s, c, s->adj, s->edg, cf) && ref_nonsingle(s, c, s->dadj, s->dedg, cf);
}
...
static int refine(struct saucy *s, struct coloring *c) {
  ...
  while (1) {
    ...
    if (s->nsinduce) {
      ...
      s->indmark[front] = 0;
      if (!s->ref_singleton(s, c, front))
        break;
    } else if (s->nninduce) {
      front = s->ninduce[--s->nninduce];
      s->indmark[front] = 0;
      if (!s->ref_nonsingle(s, c, front))
        break;
    } else {
      return 1;
    };
...
static int descend_leftmost(struct saucy *s) {
  ...
  while (!at_terminal(s)) {
    ...
    if (!descend(s, &s->left, target, min))
      return 0;
  }
...
static int zeta_fixed(struct saucy *s) { return s->ndiffs == s->nundiffs; }

static void select_decomposition(struct saucy *s, int *target, int *lmin, int *rmin) {
  ...
  for (i = 0; i < s->npairs; ++i) {
    ...
    *rmin = s->right.unlab[k];
    ...
    if (clen[*target] && in_cell_range(&s->left, *lmin, *target) && in_cell_range(&s->right, *rmin, *target))
      return;
  }
...
static int descend_left(struct saucy *s) {
  ...
  while (!at_terminal(s) && !zeta_fixed(s)) {
    ...
    if (!descend(s, &s->right, target, rmin))
      return 0;
    if (s->nsplits != s->splitlev[s->lev])
      return 0;
  }
...
static int do_backtrack(struct saucy *s) {
  ...
  return s->lev == s->anc ? backtrack_leftmost(s) : backtrack_other(s);
}
...
static int do_search(struct saucy *s) {
  ...
  while (s->lev && not timeLimitReached(s)) {
    ...
    if (descend(s, &s->right, s->start[s->lev], min) && descend_left(s)) {
      ...
      if (s->is_automorphism(s)) {
        ...
        update_theta(s);
        return s->consumer(s->n, s->gamma, s->ndiffs, s->unsupp, s->arg);
      } else {
        unprepare_permutation(s);
      }
    }
...
void saucy_search(struct saucy *s, const struct saucy_graph *g, int directed, saucy_consumer *consumer, void *arg,
                  struct saucy_stats *stats) {
  int i, j, max = 0;
...
struct saucy *saucy_alloc(int n, unsigned int tlim) {
  ...
  if (s->ninduce && s->sinduce && s->left.cfront && s->left.clen && s->right.cfront && s->right.clen && s->stuff &&
      ...
      s->undiffnons && s->difflev && s->undifflev && s->specmin) {
    return s;
  } else {
    saucy_free(s);
    return nullptr;
