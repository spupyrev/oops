...
bool Permutation::isInvolution() { return getMaxCycleSize() == 2; }

bool Permutation::permutes(uint lit) { return perm.count(lit) > 0; }

uint Permutation::supportSize() { return domain.size(); }

bool Permutation::isIdentity() { return supportSize() == 0; }

// TODO: expand this detection method with "non-binary involutions"

bool Permutation::formsMatrixWith(std::shared_ptr<Permutation> other) {
  ...
  for (auto lit : getCycleReprs()) {
    ...
    if (lit_mpd == sym_mpd || (lit == lit_mpd && sym == sym_mpd) || (lit != lit_mpd && sym != sym_mpd)) {
      return false;
    }
  }
  return true;
...
pair<std::shared_ptr<Permutation>, std::shared_ptr<Permutation>>
Permutation::getLargest(std::shared_ptr<Permutation> other) {
...
void Permutation::getSharedLiterals(std::shared_ptr<Permutation> other, vector<uint> &shared) {
  shared.clear();
  pair<std::shared_ptr<Permutation>, std::shared_ptr<Permutation>> ordered = getLargest(other);
...
uint Permutation::getNbCycles() { return getCycleReprs().size(); }

bool Permutation::equals(std::shared_ptr<Permutation> other) {
  if (supportSize() != other->supportSize() || getMaxCycleSize() != other->getMaxCycleSize() ||
...
void Group::add(std::shared_ptr<Permutation> p) {
  permutations.push_back(p);
  support.insert(p->domain.cbegin(), p->domain.cend());
}

void Group::addMatrix(std::shared_ptr<Matrix> m) {
...
std::shared_ptr<Matrix> Group::getInitialMatrix() {
  std::map<uint, std::vector<std::shared_ptr<Permutation>>> involutions; // indexed by size
  for (auto p : permutations) {
    if (p->isInvolution()) {
  ...
  std::shared_ptr<Matrix> result(new Matrix());
...
uint Group::getNbMatrices() { return matrices.size(); }
...
std::shared_ptr<Matrix> Group::getMatrix(uint idx) { return matrices.at(idx); }

void Group::addMatrices() {
  std::shared_ptr<Matrix> matrix = getInitialMatrix(); // if possible, gives an initial matrix
...
void Group::checkColumnInterchangeability(std::shared_ptr<Matrix> m) {
  // create first column
  ...
  std::shared_ptr<Matrix> newM(new Matrix());
  newM->add(first);
...
void Group::maximallyExtend(std::shared_ptr<Matrix> matrix, uint indexOfFirstNewRow) {
...
uint Group::getSize() { return permutations.size(); }
...
void Group::getDisjointGenerators(std::vector<std::shared_ptr<Group>> &subgroups) {
  // calculate maximal subsets of generators with pairwise disjoint supports
  subgroups.clear();

  if (matrices.size() > 0) {
    std::shared_ptr<Group> current(new Group());
    for (auto m : matrices) {
      current->addMatrix(m);
    }
    matrices.clear();
    ...
    while ((int)current->getSize() > previoussize) {
      ...
      for (int i = permutations.size() - 1; i >= 0; --i) {
        if (not isDisjoint(current->support, permutations[i]->posDomain)) { // it suffices to check that the positive
                                                                            // literals occur in the group support
...
bool Group::permutes(uint lit) { return support.count(lit) > 0; }

uint Group::getSupportSize() { return support.size(); }

void eliminateNonStabilizers(std::vector<std::shared_ptr<Permutation>> &permutations, uint lit) {
...
void getOrbits2(const std::vector<std::shared_ptr<Permutation>> &permutations,
                std::vector<std::shared_ptr<std::vector<uint>>> &orbits) {
  ...
  for (auto l : posSupport) {
    if (visitedLits.insert(l).second) { // lit did not yet occur in visitedLits
      std::shared_ptr<std::vector<uint>> newOrbit(new std::vector<uint>());
...
void getPosLitOccurrenceCount(const std::vector<std::shared_ptr<Permutation>> &permutations,
...
void Group::addBinaryClausesTo(Breaker &brkr, std::vector<uint> &out_order,
                               const std::unordered_set<uint> &excludedLits) {
  // now, look for literals with large orbits (of a stabilizer group for smaller literals) as first elements of the
  // order
  std::vector<std::shared_ptr<Permutation>> perms = permutations;
  while (perms.size() > 0) {
    // as long as we have some permutations stabilizing everything in order so far, continue looking for other literals
    // to add to the order
    ...
    std::vector<std::shared_ptr<std::vector<uint>>> orbs;
    getOrbits2(perms, orbs);
    std::unordered_map<uint, uint> lits2occ;
    getPosLitOccurrenceCount(perms, lits2occ);

    std::shared_ptr<std::vector<uint>> finalOrb(new std::vector<uint>());
    ...
    if (finalOrb->size() > 0) {
      ...
      for (auto l : *finalOrb) {
        ...
        brkr.addBinClause(neg(finalLit),
...
void Group::getOrderAndAddBinaryClausesTo(Breaker &brkr, std::vector<uint> &out_order) {
  ...
  matrixLits.clear(); // use this set to avoid double addition of literals occurring in more than one matrix, and avoid
                      // addition of negative literals to order
  for (auto m : matrices) {
    for (uint i = 0; i < m->nbRows(); ++i) {
...
void Group::addBreakingClausesTo(Breaker &brkr) {
  ...
  for (auto m : matrices) {
    for (uint idx = 0; idx < m->nbRows() - 1; ++idx) {
      std::shared_ptr<Permutation> rowswap(new Permutation(*m->getRow(idx), *m->getRow(idx + 1)));
...
uint Matrix::nbRows() { return rows.size(); }
...
std::vector<uint> *Matrix::getRow(uint rowindex) { return rows[rowindex]; }

void Matrix::tryToAddNewRow(std::shared_ptr<Permutation> p, uint rowIndex, Specification *theory) {
...
bool Matrix::permutes(uint x) { return rowco.count(x) > 0; }

uint Matrix::getLit(uint row, uint column) { return rows.at(row)->at(column); }

uint Matrix::getRowNb(uint x) { return rowco.at(x); }

uint Matrix::getColumnNb(uint x) { return colco.at(x); }

std::shared_ptr<Permutation> Matrix::testMembership(const std::shared_ptr<Permutation> p) {
  /**
   * NOTE:
   ...
   * A corresponding set of basic orbits is formed by taking as ith orbit (Delta_i) the first elements of row i to row
   * last. A representative function (u_i) for each ith step in the stabilizer chain then maps row j to the swap of row
   * i and j (with j>=i).
   *
  ...
  std::shared_ptr<Permutation> g = p;
...
std::shared_ptr<Permutation> Matrix::getProductWithRowsWap(const std::shared_ptr<Permutation> p, uint r1, uint r2) {
  if (r1 == r2) {
    return p;
  } else if (p->isIdentity()) {
    std::shared_ptr<Permutation> result(new Permutation(*getRow(r1), *getRow(r2)));
    return result;
  }
  ...
  std::shared_ptr<Permutation> result(new Permutation(protoPerm));
  return result;
}
} // namespace BreakID
