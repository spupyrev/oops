...
inline bool sign(uint lit) { return lit & 1; }

inline uint neg(uint lit) { return lit ^ 1; }

inline uint encode(int lit) { return (lit > 0 ? 2 * (lit - 1) : 2 * (-lit - 1) + 1); }

inline int decode(uint lit) { return (sign(lit) ? -(lit / 2 + 1) : lit / 2 + 1); }
...
public:
  ...
  void to_vector(std::vector<int> &vec) {
    vec.reserve(lits.size());
    for (auto lit : lits) {
      vec.push_back(decode(lit));
    }
  }
...
public:
  // Note: we use "constraints" as basic rules with an empty head. Thus, there is no nice correspondence between our
  // representation of rules and the lparse-smodels format Note: similarly, we use "disjunctive rules" as basic rules
  // with multiple heads. (quite standard representation of those). Again: not nice  correspondence between our
  // representation of rules and the lparse-smodels format to respresent rules. As such, there is currently not yet a
  // GOOD printing method for the rules.
  int ruleType;
  ...
  Rule(int inRuleType, const std::vector<uint> &inHeads, const std::vector<uint> &bodies, int inBound,
       std::vector<int> &inWeights)
      : hashValue(0), ruleType(inRuleType), headLits(inHeads.cbegin(), inHeads.cend()),
        bodyLits(bodies.cbegin(), bodies.cend()), bound(inBound), weights(inWeights.cbegin(), inWeights.cend()) {
...
struct UVecHash {
  ...
  size_t operator()(const std::shared_ptr<std::vector<uint>> first) const { return _getHash(*first); }

  size_t operator()(const std::shared_ptr<Clause> cl) const { return cl->getHashValue(); }

  size_t operator()(const std::shared_ptr<Rule> r) const { return r->getHashValue(); }

  size_t operator()(const std::shared_ptr<PBConstraint> r) const { return r->getHashValue(); }
};

struct UvecEqual {
  template <class T> bool equals(const std::vector<T> &first, const std::vector<T> &second) const {
  ...
  template <class T>
  bool operator()(const std::shared_ptr<std::vector<T>> first, const std::shared_ptr<std::vector<T>> second) const {
  ...
  bool operator()(const std::shared_ptr<Rule> first, const std::shared_ptr<Rule> second) const {
    return (first->ruleType == second->ruleType) && equals(first->headLits, second->headLits) &&
           equals(first->bodyLits, second->bodyLits) && equals(first->weights, second->weights) &&
           (first->bound == second->bound);
  }

  bool operator()(const std::shared_ptr<PBConstraint> first, const std::shared_ptr<PBConstraint> second) const {
    auto &lhs = first->getTerms();
    auto &rhs = second->getTerms();
    return first->getWeight() == second->getWeight() && lhs.size() == rhs.size() &&
           std::equal(lhs.begin(), lhs.end(), rhs.begin());
  }
};

template <class T> bool isDisjoint(std::unordered_set<T> &uset, std::vector<T> &vec) {
  for (T x : vec) {
    if (uset.count(x) > 0) {
      return false;
    }
...
template <class T> void swapErase(std::vector<T> &vec, uint index) {
  vec[index] = vec.back();
  vec.pop_back();
}
