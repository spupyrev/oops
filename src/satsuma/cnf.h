...
class cnf {
private:
  tsl::robin_set<std::vector<int>, satsuma::any_hash> clause_db_arbitrary;
  tsl::robin_set<std::pair<int, int>, satsuma::pair_hash> clause_db_size2;
  tsl::robin_set<std::tuple<int, int, int>, satsuma::triple_hash> clause_db_size3;
...
public:
  void read_from_cnf2wl(cnf2wl &other_formula) {
    ...
    for (int i = 0; i < other_formula.n_clauses(); ++i) {
      ...
      for (int j = 0; j < other_formula.clause_size(i); ++j) {
        ...
        if (assigned == 0)
          next_clause.push_back(lit);
      }
      if (!satisfied)
        add_clause(next_clause);
    }
  }
  ...
  bool read_db(std::vector<int> &clause) const {
    ...
    if (min_v != -1 && min_v_size < 128) {
      for (auto const c : variable_used_list[min_v]) {
        ...
        if (pt_end - pt_start != static_cast<int>(clause.size()))
          continue;
        int j = 0;
        for (int i = pt_start; i < pt_end; ++i) {
          if (clauses[i] != clause[j])
            break;
          ++j;
        }
        if (j == static_cast<int>(clause.size()))
          return true;
      }
      return false;
    } else {
  ...
  int vertex_variable_occurence(int v) { return variable_used_list[abs(graph_to_sat(v)) - 1].size(); }
  ...
  bool is_automorphism(int domain_size, dejavu::groups::automorphism_workspace &automorphism) {
    ...
    for (auto c : need_to_test_clauses) {
      ...
      int j = 0;
      for (; j < clause_size(c); ++j)
        if (test_clause[j] != literal_at_clause_pos(c, j))
          break;
      if (j == clause_size(c))
        continue;

      if (!read_db(test_clause))
        return false;
    }
  ...
  void dimacs_output_clauses_to(std::vector<std::vector<int>> &clauses) {
    std::vector<int> cl;
