...
public:
  ...
  static void shallow_bfs_invariant(sgraph *g, ir::controller &local_state, worklist_t<uint64_t> &inv,
                                    groups::orbit &orbit_partition, int depth = 8, bool lower_depth = true) {
    ...
    while (repeat) {
      ...
      for (int i = 0; i < g->v_size; ++i) {
        if (!orbit_partition.represents_orbit(i))
          continue;
      ...
      for (int i = 0; i < g->v_size; ++i)
        inv[i] = inv[orbit_partition.find_orbit(i)];
    }
        ...
        for (int _col = 0; _col < g->v_size;) {
          ...
          if (_col_sz >= 2 && _col_sz <= col_sz_upper_bound && !original_colors.get(_col)) {
            ...
            for (int jj = _col; jj < _col + _col_sz; ++jj)
              col_buffer.push_back(local_state.c->lab[jj]);
   ...
   */
  void set_splits_hint(int splits_hint) { h_splits_hint = splits_hint; }
  ...
  bool inprocess(sgraph *g, ir::shared_tree &tree, groups::compressed_schreier &group, ir::controller &local_state,
                 ir::limited_save &root_save, int, bool use_bfs_inprocess, bool use_shallow_inprocess,
                 bool use_shallow_quadratic_inprocess, groups::orbit &orbit_partition) {
