 ...
 *
 * Performs random walks of the IR shared_tree, sifting resulting automorphisms into the given Schreier structure. If
 * the Schreier structure is complete with respect to the base, or the probabilistic abort criterion satisfied, the
...
class random_ir {
  ...
  static void load_state_from_leaf(sgraph *g, ir::controller &local_state, ir::limited_save &start_from,
    ...
    for (int i = 0; i < leaf->get_lab_or_base_size(); ++i)
      base.push_back(leaf->get_lab_or_base()[i]);
  ...
  bool add_leaf_to_storage_and_group(sgraph *g, dejavu_hook *hook, groups::compressed_schreier &group,
    ...
    for (int hash_offset = 0; hash_offset < h_hash_col_limit; ++hash_offset) {
      ...
      if (hash_offset == 1)
        local_state.write_strong_invariant_quarter(g);
      ...
      if (cert) {
        ...
        s_support_limit_reached = (h_schreier_support_limit >= 0) && (group.get_support() > h_schreier_support_limit);

        // output the automorphism
        if (hook)
          (*hook)(g->v_size, gl_automorphism.p(), gl_automorphism.nsupp(), gl_automorphism.supp());
...
public:
  ...
  void use_look_close(bool look_close = false) { h_look_close = look_close; }
   ...
   */
  random_ir(timed_print &printer, groups::schreier_workspace &schreier, groups::automorphism_workspace &automorphism,
  ...
  static bool h_almost_done(groups::compressed_schreier &group) { return group.get_consecutive_success() >= 2; }

  static void specific_walk(sgraph *g, ir::shared_tree &ir_tree, ir::controller &local_state,
  ...
  void random_walks(sgraph *g, dejavu_hook *hook, std::function<ir::type_selector_hook> *selector,
    ...
    while (!group.probabilistic_abort_criterion() && !group.deterministic_abort_criterion() &&
      ...
      if (s_paths_failany > 8 && (s_paths & 0x00000FFF) == 0x000000FE)
        gl_printer.progress_current_method("random", "leaves", ir_tree.stat_leaves(), "f1", s_paths_fail1, "compress",
      ...
      if (local_state.s_base_pos <= could_start_from) {
        ...
        if (progress_now - progress_initial > 0.1) {
          gl_printer.progress_current_method("random", "root_cells", 1.0 * s_cells_now / g->v_size, "base_pos",
                                             could_start_from, "sift", s_sifting_success, "rsift",
      ...
      while (g->v_size != local_state.c->cells) {
        ...
        if (base_pos > start_from_base_pos + 1 && g->v_size > 5000 && s_sifting_success >= 0 &&
            s_random_sift_success < 0 && base_pos < static_cast<int>(local_state.compare_base_vertex->size())) {
          // or even better: let's choose the base vertex, if it's in the correct color
          const int v_base = (*local_state.compare_base_vertex)[base_pos];
          const int v_base_col = local_state.c->vertex_to_col[v_base];
        ...
        if (base_pos == target_level) {
          ...
          s_rolling_first_level_success = (9.0 * s_rolling_first_level_success + (local_state.T->trace_equal())) / 10.0;
          if (!h_look_close && !local_state.T->trace_equal())
            break;
        }
      ...
      const bool sift = add_leaf_to_storage_and_group(g, hook, group, ir_tree.stored_leaves, local_state, other_state,
                                                      *root_save, uniform);
