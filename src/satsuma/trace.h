...
private:
  ...
  void write_compare_no_limit(int d) {
    ...
    if (record)
      data.push_back(d);
    if (compare)
      comp = comp && (position < ((int)compare_trace->data.size())) && (compare_trace->data[position] == d);
    ++position;
  }
...
public:
  trace *get_compare_trace() { return compare_trace; }
  ...
  void op_additional_info(int d) { write_compare(d); }
  ...
  dej_nodiscard bool blueprint_is_next_cell_active() {
    if (!compare || !comp || position > static_cast<int>(compare_trace->data.size()))
      return true;
   ...
   * @param compare_trace
   */
  void set_compare_trace(trace *new_compare_trace) { this->compare_trace = new_compare_trace; }
   ...
   * @param compare
   */
  void set_compare(bool new_compare) { this->compare = new_compare; }
   ...
   */
  void set_hash(uint64_t new_hash) { this->hash = new_hash; }

  /**
   * @return Whether the recorded operations deviated from the stored trace in \a compare_trace.
   */
  dej_nodiscard bool trace_equal() const { return comp; }

  /**
   * Resets the trace to find new deviations from the stored trace.
   */
  void reset_trace_equal() { comp = true; }
  /**
   * Resets the trace to find new deviations from the stored trace.
   */
  void reset_trace_unequal() { comp = false; }
  ...
  // record to trace
  void set_record(bool new_record) { this->record = new_record; }

  void reserve(const int n) { data.reserve(n); }
  ...
  dej_nodiscard int get_position() const { return position; }
};
} // namespace ir
} // namespace dejavu
