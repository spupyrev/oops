...
class sgraph {
  struct vertexComparator {
    ...
    bool operator()(const int &v1, const int &v2) const { return graph.d[v1] < graph.d[v2]; }
  };
...
public:
   ...
   */
  static_graph(const int nv, const int ne) { initialize_graph(nv, ne); };

  static_graph() = default;

  static_graph(const static_graph &other) { copy(&other); }

  ~static_graph() { dealloc(); }
  ...
  void initialize_graph(const int nv, const int ne) {
    ...
    if (ne < 0)
      throw std::out_of_range("number of edges must be positive");
  ...
  void sanity_check() { g.sanity_check(); }
...
static inline bool parse_dimacs(const std::string &filename, dejavu::sgraph *g, int **colmap, bool silent = true,
  ...
  const double parse_time =
      (double)(std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::high_resolution_clock::now() - timer)
                   .count());
  if (!silent)
    std::cout << std::setprecision(2) << "parse_time=" << parse_time / 1000000.0 << "ms";
