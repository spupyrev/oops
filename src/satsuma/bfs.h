...
public:
  ...
  void compute_node(sgraph *g, dejavu_hook *hook, ir::shared_tree *ir_tree, ir::controller &local_state,
    ...
    const int parent_node_base_vert =
        parent_node_base_pos >= 0 ? node->get_save()->get_base()[parent_node_base_pos] : -1;
    ...
    if (local_state.T->trace_equal() && cert) {
      ...
      if (local_state.s_base_pos > 1) {
        ...
        ++s_total_prune;
        if (parent_is_base)
          ir_tree->stored_deviation.record_deviation(local_state.T->get_hash());
        else {
