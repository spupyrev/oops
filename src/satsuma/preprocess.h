...
public:
  ...
  explicit preprocessor(dejavu::ir::refinement *R) { R1 = R; }
  ...
  void multiply_to_group_size(int n) { grp_sz.multiply(n); }
...
private:
  ...
  void meld_translation_layers() {
    ...
    for (int i = 0; i < reduced_size; ++i) {
      ...
      for (auto v : recovery_strings[orig_i])
        baked_recovery_string.push_back(v);
      ...
      for (int j = 0; j < color_size; ++j) {
        ...
        for (int k = 0; k < g->d[vertex]; ++k) {
          ...
          for (int l = 0; l < g->d[neighbour]; ++l) {
            ...
            if (touched.get(neighbour_neighbour))
              continue;
        ...
        for (int other_vertex : potential_twin_list) {
          ...
          if (potential_twin_counter[other_vertex] < g->d[vertex] - 1)
            continue;
          if (vertex == other_vertex || del.get(other_vertex) ||
              c.vertex_to_col[other_vertex] != c.vertex_to_col[vertex])
            continue;
      ...
      if (g->d[test_v] == 2) {
        ...
        if (g->d[n1] != 2 && g->d[n2] != 2) {
          ...
          if (touched_color_cache.get(col_n1) && touched_color_cache.get(col_n2) && already_matched_n1_n2 &&
              worklist_deg0[col_n1] == 1 && worklist_deg0[col_n2] == 1) {
      ...
      for (int j = 0; j < color_size; ++j) {
        ...
        for (int k = 0; k < num_paths; ++k) {
          ...
          for (int l = 0; l < path.cur_pos; ++l) {
            ...
            recovery_strings[unique_endpoint_orig].insert(recovery_strings[unique_endpoint_orig].end(),
  ...
  void recovery_attached_edges_to_automorphism(int v, int *aut, dejavu::ds::worklist *aut_supp,
                                               dejavu::ds::worklist *help_array1, dejavu::ds::worklist *help_array2) {
    if (recovery_edge_adjacent[v].empty())
      return;
  ...
  void red_deg2_densify(dejavu::sgraph *g, int *colmap) {
    ...
    for (int i = 0; i < g->v_size;) {
      ...
      for (int j = 0; j < endpoints; ++j) {
        ...
        if (color_unique.get(neighbour_col) && color != endpoint_col && endpoint_col_sz >= color_size) { // if unique
          filter.push_back(j); // add index to filter
        }
      }
  ...
  void red_deg2_densifysc1(dejavu::sgraph *g, int *colmap) {
    ...
    for (int i = 0; i < g->v_size;) {
      ...
      if (!only_one || color_deg_single == 0)
        continue;
      ...
      if (g->d[i] == 2) {
        ...
        if (cycle_length == -1) {
          ...
          for (int j = 0; j < path.cur_pos; ++j) {
            colmap[path[j]] =
                colmap[path[j]] + g->v_size * cycle_length; // maybe this should receive a more elegant solution
          }
        }
      }
    }
            ...
            if (all_unique && color < endpoint_col) {
              ...
              for (int jjj = 0; jjj < color_size; ++jjj) {
                ...
                for (int jj = 0; jj < neighbour_list.cur_pos; ++jj) {
                  ...
                  path.reset();
                  int _endpoint = walk_to_endpoint_collect_path(g, _neighbour, col1_vertj, &path);
                  ...
                  recovery_strings[vert_orig].reserve(recovery_strings[vert_orig].size() + path.cur_pos);
                  for (int l = 0; l < path.cur_pos; ++l) {
                    dej_assert(path[l] >= 0);
                    dej_assert(path[l] < g->v_size);
                    const int path_v_orig = translate_back(path[l]);
                  ...
                  recovery_strings[endpoint_orig].reserve(recovery_strings[endpoint_orig].size() + path.cur_pos);
                  for (int l = 0; l < path.cur_pos; ++l) {
                    dej_assert(path[l] >= 0);
                    dej_assert(path[l] < g->v_size);
                    const int path_v_orig = translate_back(path[l]);
      ...
      if (is_pairs) {
        ...
        for (int j = 0; j < parentlist.cur_pos; ++j) {
          ...
          for (size_t s = 0; s < recovery_strings[original_pair_to].size(); ++s)
            recovery_strings[original_parent].push_back(recovery_strings[original_pair_to][s]);
          ...
          while (!stack1.empty()) {
            ...
            for (int f = from; f < to; ++f) {
              ...
              for (size_t s = 0; s < recovery_strings[orig_next].size(); ++s)
                recovery_strings[original_parent].push_back(recovery_strings[orig_next][s]);
              if (from_next != to_next)
                stack1.push_back(std::pair<int, int>(from_next, to_next));
            }
          }
    ...
    if (g->v_size > 1) {
      ...
      for (int _i = 0; _i < g->v_size; ++_i) {
        if (!del.get(_i) && childcount[_i] > 0 && c.ptn[c.vertex_to_col[_i]] > 0) {
          ...
          recovery_strings[orig_i].reserve(recovery_strings[orig_i].size() + childcount[_i]);
          while (!stack1.empty()) {
            std::pair<int, int> from_to = stack1.pop_back();
            int from = from_to.first;
            const int to = from_to.second;
            if (from == to) {
              ...
              for (size_t j = 0; j < recovery_strings[orig_next].size(); ++j)
                recovery_strings[orig_i].push_back(recovery_strings[orig_next][j]);
    ...
    for (int i = 0; i < g->v_size;) {
      ...
      for (int ii = 0; ii < c.ptn[i] + 1; ++ii) {
        ...
        for (int f = g->v[vx]; f < g->v[vx] + g->d[vx]; ++f) {
          const int v_neigh = g->e[f];
          if (worklist_deg0[c.vertex_to_col[v_neigh]] == c.ptn[c.vertex_to_col[v_neigh]] + 1) {
  ...
  void pre_hook(int, const int *_aut, int _supp, const int *_aut_supp, dejavu_hook *hook) {
    ...
    if (!recovery_edge_attached.empty()) {
      for (int i = 0; i < end_pos; ++i) {
        recovery_attached_edges_to_automorphism(automorphism_supp[i], automorphism.get_array(), &automorphism_supp,
                                                &aux_automorphism, &before_move);
      }
    }
...
private:
  ...
  void order_according_to_color(dejavu::sgraph *g, int *colmap) {
    bool in_order = true;
    for (int i = 0; i < g->v_size - 1 && in_order; ++i)
...
public:
  ...
  void reduce(dejavu::sgraph *g, int *colmap, dejavu_hook *hook, const std::vector<preop> *schedule = nullptr) {
    const std::vector<preop> default_schedule = {deg01,  qcedgeflip, deg01,   twins,
                                                 deg2ma, deg2ue,     densify2}; // deg2ma, deg2ue, densify2
    ...
    if (k == g->v_size && l == g->v_size) {
      ...
      if (print)
        print->timer_print("regular", g->v_size, g->e_size);
      return;
    }
    ...
    if (g->v_size > 64)
      ...
      order_edgelist(g);
    if (print)
      print->timer_print("order", g->v_size, g->e_size);
    g->initialize_coloring(&c, colmap);
    ...
    if (c.cells == g->v_size) {
      ...
      g->e_size = 0;
      if (print)
        print->timer_print("discrete", g->v_size, g->e_size);
      return;
    }
    ...
    copy_coloring_to_colmap(&c, colmap);
    if (print)
      print->timer_print("colorref", g->v_size, c.cells);
    ...
    if (!has_deg_0 && !has_deg_1 && !has_deg_2 && !has_discrete)
      return;
    ...
    if (schedule != nullptr) {
      ...
      for (size_t pc = 0; pc < schedule->size(); ++pc) {
        ...
        switch (next_op) {
        case preop::deg01: {
          if (!has_deg_0 && !has_deg_1 && !has_discrete && !graph_changed)
            break;
          red_deg10_assume_cref(g, colmap, hook);
          perform_del(g, colmap);
          if (print)
            print->timer_print("deg01", g->v_size, g->e_size);
        ...
        case preop::deg2ma: {
          if (!has_deg_2 && !graph_changed)
            break;
          red_deg2_path_size_1(g, colmap);
          perform_del_add_edge(g, colmap);
          if (print)
            print->timer_print("deg2ma", g->v_size, g->e_size);
          dej_assert(_automorphism_supp.cur_pos == 0);
        ...
        case preop::twins: {
          ...
          if (print)
            print->timer_print("twins", g->v_size, g->e_size);
          break;
        }
        case preop::deg2ue: {
          if (!has_deg_2 && !graph_changed)
          ...
          if (print)
            print->timer_print("deg2ue", g->v_size, g->e_size);
          dej_assert(_automorphism_supp.cur_pos == 0);
          break;
        }
        case preop::densify2: {
          ...
          if (print)
            print->timer_print("densify2", g->v_size, g->e_size);
          dej_assert(_automorphism_supp.cur_pos == 0);
          break;
        }
        case preop::qcedgeflip: {
          ...
          if (print)
            print->timer_print("qcedgeflip", g->v_size, g->e_size);
          break;
        }
        }
  ...
  void save_my_hook(dejavu_hook *hook) { saved_hook = hook; }
  ...
  void traces_save_my_preprocessor() { save_preprocessor = this; }
  ...
  void nauty_save_my_preprocessor() { save_preprocessor = this; }
