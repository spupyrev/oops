...
public:
  ...
  int literal_to_number_of_binary_clauses(int literal) { return literal_to_binary_clauses[sat_to_graph(literal)]; }

  int literal_to_number_of_hyperedges(int literal) { return literal_to_hyperedges[sat_to_graph(literal)]; }

  int literal_to_number_of_removed_uses(int literal) { return literal_to_removed[sat_to_graph(literal)]; }

  int literal_to_number_of_ternary_clauses(int literal) { return literal_to_ternary_clauses[sat_to_graph(literal)]; }
  ...
  bool is_clause_removed(int i) { return clause_tombstone[i]; }

  int n_hyperedges() { return hyperedge_list.size(); }
  ...
  void hypergraph_reduction() {
    ...
    for (int i = 0; i < wrapped_formula.n_clauses(); ++i) {
      ...
      for (int f1 = 0; f1 <= 1; ++f1) {
        for (int f2 = 0; f2 <= 1; ++f2) {
          for (int f3 = 0; f3 <= 1; ++f3) {
            ...
            if (f1 == 1)
              test_clause[0] = -test_clause[0];
            if (f2 == 1)
              test_clause[1] = -test_clause[1];
            if (f3 == 1)
              test_clause[2] = -test_clause[2];
      ...
      int excluded_negation_symmetry =
          (iso_inv(l1) != iso_inv(-l1)) + (iso_inv(l2) != iso_inv(-l2)) + (iso_inv(l3) != iso_inv(-l3));
      ...
      if (flips_exist == 4 && flips[0] == 0 && flips[1] == 2 && flips[2] == 2 && flips[3] == 2 &&
        ...
        if (canonical) {
          std::vector<int> hyperedge;
          for (int k = 0; k < 3; ++k) {
            if (iso_inv(orig_clause[k]) < iso_inv(-orig_clause[k])) {
              hyperedge.push_back(orig_clause[k]);
            } else if (iso_inv(-orig_clause[k]) < iso_inv(orig_clause[k])) {
              hyperedge.push_back(-orig_clause[k]);
            } else {
              hyperedge.push_back(orig_clause[k]);
            }
    ...
    std::clog << "c\t +hyedge=" << n_hyperedges() << ", +hyedge_sup=" << hyperedge_support
              << ", -cl=" << removed_clauses << ", -cl_sup=" << removed_clause_support << std::endl;
    // std::clog << "c\t found " << replacement_possible.size() << " [clauses: " << potential_removed_clauses << "]
    // potential hypergraph reductions" << std::endl;
  }
};
} // namespace satsuma
