...
static int quotient_components(sgraph *g, int *colmap, ds::worklist *vertex_to_component) {
  ...
  for (int i = 0; i < g->v_size; ++i) {
    ...
    if (total_size == g->v_size || one_component)
      break;
  }

  if (one_component) {
    for (int i = 0; i < g->v_size; ++i)
      (*vertex_to_component)[i] = 0;
  }
...
public:
  ...
  int map_back(int component, int vertex) {
    ...
    dej_assert(component_to_backward_translation[component] + vertex < static_cast<int>(backward_translation.size()));
    return (num_components <= 1 ? vertex : backward_translation[component_to_backward_translation[component] + vertex]);
  }
  ...
  void decompose(sgraph *g, int *colmap, ds::worklist &vertex_to_component, int new_num_components) {
    ...
    for (int v = 0; v < g->v_size; ++v)
      original_v.push_back(g->v[v]);
    for (int v = 0; v < g->v_size; ++v)
      original_d.push_back(g->d[v]);
   ...
   */
  sgraph *get_component(int i) { return &components_graph[i]; }
   ...
   */
  int *get_colmap(int i) { return components_coloring[i]; }
};
} // namespace ir
} // namespace dejavu
