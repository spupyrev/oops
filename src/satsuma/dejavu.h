...
public:
  void add_hook(dejavu_hook *hook) { hooks.push_back(hook); }

  void clear() { hooks.clear(); }

  dej_nodiscard size_t size() const { return hooks.size(); }
...
public:
  ...
  explicit strong_certification_hook(sgraph &g, dejavu_hook *call_hook) { initialize(g, call_hook); }
...
#else
    my_hook = std::bind(&strong_certification_hook::hook_func, this, std::placeholders::_1, std::placeholders::_2,
                        std::placeholders::_3, std::placeholders::_4);
#endif
    return &my_hook;
  }
...
class schreier_hook : public hook_interface {
  ...
  void hook_func(int n, const int *p, int nsupp, const int *supp) { my_schreier.sift(n, p, nsupp, supp); }
...
public:
   ...
   */
  void set_error_bound(int error_bound = 10) { h_error_bound = error_bound; }
   ...
   */
  int get_error_bound() const { return h_error_bound; }
   ...
   */
  void set_true_random(bool use_true_random = true) { h_random_use_true_random = use_true_random; }
   ...
   */
  void set_prefer_dfs(bool prefer_dfs = true) { h_prefer_dfs = prefer_dfs; }
   ...
   */
  void set_pseudo_random(bool use_pseudo_random = true) { h_random_use_true_random = !use_pseudo_random; }

  /**
   * Seed to use for pseudo random number generation.
   * @param seed the seed
   */
  void set_seed(int seed = 0) { h_random_seed = seed; }
   ...
   */
  void set_limit_component(long limit_component = -1) { h_limit_component = limit_component; }
   ...
   */
  void set_limit_budget(int limit_budget = -1) { h_limit_budget = limit_budget; }
   ...
   */
  void set_limit_schreier_support(int limit_support = -1) { h_limit_schreier_support = limit_support; }
   ...
   */
  void set_disallow_alteration(bool may_alter_graph = false) { h_destructive = may_alter_graph; }
   ...
   */
  void set_decompose(bool use_decompose = true) { h_decompose = use_decompose; }
   ...
   */
  void set_print(bool print = true) { h_silent = !print; }

  /**
   * How large was the automorphism group computed?
   * @return the automorphism group size
   */
  dej_nodiscard big_number get_automorphism_group_size() const { return s_grp_sz; }
   ...
   */
  dej_nodiscard bool get_deterministic_termination() const { return s_deterministic_termination; }
   ...
   */
  dej_nodiscard bool get_reached_limit() const { return s_limit_reached; }
    ...
    for (int i = 0; i < s_num_components; ++i) {
      ...
      while (true) {
        ...
        if (s_restarts > 0) {
          ...
          if (s_inproc_success >= 6)
            s_random_budget_bias = 0.01; // do even less random search
          if (s_inprocessed)
            h_budget = 1;        /*< if we inprocessed, we hope that the graph is easy again */
        ...
        const bool s_too_big =
            (s_inproc_success <= (s_regular + !s_prunable)) && (s_last_tree_sz < s_tree_estimate) && (s_restarts >= 2);
        ...
        for (auto &bi : local_state.base)
          base_sizes.push_back(bi.color_sz); // TODO clean up
        ...
        dfs_level = s_last_base_eq
                        ? dfs_level
                        : m_dfs.do_paired_dfs(hook, g, local_state_left, local_state,
                                              m_inprocess.inproc_maybe_individualize, base_size > 1 || s_restarts > 0);
        ...
        const bool reset_prob = sh_schreier.reset(&m_compress, g->v_size, schreierw, base_vertex, base_sizes, dfs_level,
                                                  can_keep_previous, m_inprocess.inproc_fixed_points);
        if (reset_prob)
          sh_schreier.reset_probabilistic_criterion(); /*< need to reset probabilistic abort
        ...
        while (!do_a_restart && !finished_symmetries) {
          ...
          const int s_bfs_next_level_nodes = search_strategy::bfs_ir::next_level_estimate(sh_tree, selector);
          ...
          if (s_have_rand_estimate) {
            s_path_fail1_avg = static_cast<double>(m_rand.s_paths_fail1) / static_cast<double>(m_rand.s_paths);
            s_trace_cost1_avg = static_cast<double>(m_rand.s_trace_cost1) / static_cast<double>(m_rand.s_paths);
          }
          ...
          if (s_have_rand_estimate) {
            ...
            if (sh_tree.get_finished_up_to() >= 1)
              score_bfs *= (1 - s_path_fail1_avg);
          ...
          const long s_bfs_est_mem =
              static_cast<long>(round(s_bfs_next_level_nodes * (1 - s_path_fail1_avg) * g->v_size));
          if (next_routine == bfs_ir && (s_bfs_est_mem > h_bfs_memory_limit))
            next_routine = random_ir;

          // let's stick to the budget...
          if (s_cost > h_budget)
            next_routine = restart; /*< we exceeded our budget, restart */
          ...
          if (next_routine == restart && 2 * base_size > s_bfs_next_level_nodes && s_trace_cost1_avg < base_size &&
          ...
          if (m_rand.s_rolling_success > 0.1 && s_cost <= h_budget * 4)
            next_routine = random_ir;
          if (m_rand.s_rolling_success > 0.2 && s_cost <= h_budget * 6)
            next_routine = random_ir;
          if (s_hard && m_rand.s_succeed >= 1 && s_cost <= m_rand.s_succeed * h_budget * 10 && next_routine == restart)
            next_routine = random_ir;
          ...
          if (last_routine == random_ir && next_routine != random_ir) {
            ...
            if (sh_schreier.s_densegen() + sh_schreier.s_sparsegen() > 0) {
              m_printer.timer_print("schreier",
                                    "s" + std::to_string(sh_schreier.s_sparsegen()) + "/d" +
                                        std::to_string(sh_schreier.s_densegen()),
                                    "_");
            }
          }
          ...
          case random_ir: { // random leaf search
            ...
            const bool h_look_close = ((m_rand.s_rolling_first_level_success > 0.5) && !s_short_base) ||
                                      (s_have_rand_estimate && sh_tree.get_finished_up_to() == base_size - 1);
            ...
            // decision making)
            h_rand_fail_lim_total += static_cast<int>(std::max(5.0, h_rand_fail_lim_now * s_random_budget_bias));
            ...
            if (sh_tree.get_finished_up_to() == 0 || (s_long_base && !s_any_bfs_pruned)) {
              ...
              m_rand.random_walks(g, hook, selector, sh_tree, sh_schreier, local_state, local_state_left,
                                  h_rand_fail_lim_total);
            } else {
              // random automorphisms, sampled from current bfs level of ir tree
              m_rand.random_walks_from_tree(g, hook, selector, sh_tree, sh_schreier, local_state, local_state_left,
                                            h_rand_fail_lim_total);
            }
            ...
            if (sh_tree.get_finished_up_to() == base_size) {
              ...
              if (base_size == 2) {
                ...
                m_inprocess.s_grp_sz.multiply(
                    static_cast<double>(sh_tree.h_bfs_top_level_orbit.orbit_size(base_vertex[0])) *
                        sh_tree.h_bfs_automorphism_pw,
                    0);
              } else if (base_size == 1) {
                m_inprocess.s_grp_sz.multiply(sh_tree.h_bfs_top_level_orbit.orbit_size(base_vertex[0]), 0);
              } else {
                // base case which just multiplies the remaining elements of the level
                m_inprocess.s_grp_sz.multiply(sh_tree.get_level_size(sh_tree.get_finished_up_to()), 0);
              }
            }
        ...
        const bool h_use_shallow_inprocess =
            !h_used_shallow_inprocess && s_inproc_success == 0 && s_path_fail1_avg > 0.1;
        const bool h_use_shallow_inprocess_quadratic = !h_used_shallow_inprocess_quadratic &&
                                                       s_inproc_success >= 1 + s_regular && s_prunable &&
                                                       2 * local_state.c->cells > sqrt(g->v_size) && s_hard;
        h_used_shallow_inprocess = h_used_shallow_inprocess || h_use_shallow_inprocess;
        h_used_shallow_inprocess_quadratic = h_used_shallow_inprocess_quadratic || h_use_shallow_inprocess_quadratic;

        // inprocess
        s_inprocessed =
            m_inprocess.inprocess(g, sh_tree, sh_schreier, local_state, root_save, h_budget, s_any_bfs_pruned,
      ...
      if (s_term != t_bfs && s_term != t_inproc)
