...
public:
  ...
  static bool certify_automorphism(markset &scratch_set, sgraph *g, const int *p) {
    if (!bijection_check(scratch_set, g->v_size, p))
      return false;
  ...
  static bool certify_automorphism_sparse(markset &scratch_set, const sgraph *g, const int *p, int supp,
                                          const int *supp_arr) {
    int i, found;
    if (!cycle_check(scratch_set, p, supp, supp_arr))
      return false;
...
public:
   ...
   * @param worklist_hook Function pointer that is called whenever a color class is considered for refinement. Return
   * value can be used to skip refinement of that color class.
   */
...
public:
   ...
   * @return
   */
  static int individualize_vertex(coloring *c, int v,
                                  const std::function<type_split_color_hook> &split_hook = nullptr) {
...
private:
  void report_split_color_class_first(coloring *c, int old_class, int new_class, int new_class_sz, bool is_largest) {
    c->cells += (old_class != new_class);
    dej_assert(c->ptn[new_class] + 1 == new_class_sz);
    ...
    if (!is_largest && old_class != new_class) {
      ...
      if (old_class != new_class) {
        cell_todo.add_cell(queue_pointer, old_class);
        if (c->ptn[old_class] + 1 == 1)
          singleton_hint.push_back(old_class);
      }
    }
  }
...
private:
  ...
  void refine_color_class_sparse(sgraph *g, coloring *c, int color_class, int class_size) {
  ...
  void refine_color_class_dense_cell(sgraph *g, coloring *c, int color_class, int class_size) {
    ...
    for (int _col = 0; _col < g->v_size;) {
      ...
      const int col = _col;
      _col += col_sz;
      if (col_sz == 1 || !scratch_set.get(col))
        continue;
  ...
  void refine_color_class_dense_dense(sgraph *g, coloring *c, int color_class, int class_size) {
    ...
    if (class_size == 1 && deg == g->v_size - 1)
      return; // special code for universal singletons
