...
public:
   ...
   */
  coloring *get_coloring() { return &c; }

  /**
   * @return hash of invariant of this IR node
   */
  dej_nodiscard uint64_t get_invariant_hash() const { return invariant; }

  /**
   * @return position of trace of this IR node
   */
  dej_nodiscard int get_trace_position() const { return trace_position; }

  /**
   * @return length of base of this IR node
   */
  dej_nodiscard int get_base_position() const { return base_position; }

  /**
   * @return base of this IR node
   */
  std::vector<int> &get_base() { return base_vertex; }
};
...
private:
  ...
  bool split_hook(const int old_color, const int new_color, const int new_color_sz) {
    ...
    if (!s_individualize && old_color != new_color)
      T->op_refine_cell_record(new_color);
    ...
    const bool continue_cell_limit =
        !((mode != IR_MODE_RECORD_TRACE) && T->trace_equal() &&
          s_base_pos - 1 < static_cast<int>(compare_base->size()) && (*compare_base)[s_base_pos - 1].cells == c->cells);
    ++s_splits;
...
#ifndef dej_nolambda
    return [this](auto &&PH1, auto &&PH2, auto &&PH3) {
      return this->split_hook(std::forward<decltype(PH1)>(PH1), std::forward<decltype(PH2)>(PH2),
  ...
  void color_fix_difference(const controller &state, int col) {
    ...
    for (int i = 0; i < col_sz; ++i)
      if (!diff_tester.get(c->lab[col + i]))
        add_diff_vertex(c->lab[col + i]);

    diff_tester.reset();
    for (int i = 0; i < col_sz; ++i)
      diff_tester.set(c->lab[col + i]);
    for (int i = 0; i < col_sz; ++i)
      if (!diff_tester.get(state.c->lab[col + i]))
        add_diff_vertex(state.c->lab[col + i]);
  }
...
public:
  ...
  int get_number_of_splits() { return s_splits; }
   ...
   */
  void mode_compare_base() { mode = ir::IR_MODE_COMPARE_TRACE_REVERSIBLE; }
   ...
   */
  dej_nodiscard int get_diff_diverge() const { return diff_diverge; }
  ...
  bool update_diff_vertices_last_individualization(const controller &other_state) {
    ...
    for (i = base.back().touched_color_list_pt; i < touched_color_list.cur_pos; ++i) {
      ...
      diff_original_size[old_color] = std::max(diff_original_size[old_color], new_color + c->ptn[new_color] + 1);
    }
    ...
    for (i = base.back().touched_color_list_pt; i < touched_color_list.cur_pos; ++i) {
      ...
      for (int j = old_color; j < old_color_end_pt;) {
        const int next_color = j;
        const int next_color_sz = c->ptn[next_color] + 1;
        if (j != largest_fragment)
          color_fix_difference(other_state, next_color);
        j += next_color_sz;
      }

      // unless the largest fragment is a singleton, in that case we want to record the singleton
      if (largest_fragment_sz == 1)
        color_fix_difference(other_state, largest_fragment);
    }
  ...
  int diff_num() { return diff_vertices_list.cur_pos; };
   ...
   */
  void use_increase_deviation(bool deviation_inc_active) { h_deviation_inc_active = deviation_inc_active; }
   ...
   */
  void set_increase_deviation(int deviation_inc = 48) { h_deviation_inc = deviation_inc; }
  ...
  void save_reduced_state(limited_save &state) {
    state.save(base_vertex, *c, T->get_hash(), T->get_position(), s_base_pos);
  }
  ...
  void load_reduced_state(limited_save &state) {
    ...
    if (mode == IR_MODE_COMPARE_TRACE_REVERSIBLE)
      reset_touched();
  }
  ...
  dej_nodiscard coloring *get_coloring() const { return c; }

  dej_nodiscard int get_base_pos() const { return s_base_pos; }
   ...
   */
  void write_to_trace(const int d) const { T->op_additional_info(d); }
  ...
  void move_to_child(sgraph *g, int v) {
    ...
    if (mode != IR_MODE_COMPARE_TRACE_IRREVERSIBLE)
      base.emplace_back(prev_col, prev_col_sz, c->cells, touched_color_pt, singleton_pt, trace_pos, trace_hash);
  }
   ...
   * @param g The graph.
   */
  void refine(sgraph *g) { R->refine_coloring_first(g, c); }
  ...
  bool certify(sgraph *g, groups::automorphism_workspace &automorphism) {
    if (automorphism.nsupp() == 0)
      return true;
    if (automorphism.nsupp() > g->v_size / 4) {
      return R->certify_automorphism(g, automorphism.p());
    } else {
      return R->certify_automorphism_sparse(g, automorphism.p(), automorphism.nsupp(), automorphism.supp());
    }
  }
};
...
class cell_selector_factory {
  ...
  static int color_score_size(controller *state, int color) { return state->get_coloring()->ptn[color]; }
...
public:
  ...
  std::function<type_selector_hook> *get_selector_hook() {
    dynamic_seletor =
        std::bind(&cell_selector_factory::cell_selector, this, std::placeholders::_1, std::placeholders::_2);
   ...
   */
  big_number get_ir_size_estimate() { return ir_tree_size_estimate; }
    ...
    while (state->get_coloring()->cells != g->v_size) {
      ...
      if (best_color == -1) {
        ...
        for (int i = start_test_from; i < state->get_coloring()->domain_size && test_lim >= 0;) {
          ...
          if (col_sz == 0 && start_test_from == i)
            start_test_from += col_sz + 1;
          i += col_sz + 1;
        }
      }
  ...
  void find_sparse_optimized_base_recurse(sgraph *g, controller *state, int perturbe) {
    ...
    while (state->get_coloring()->cells != g->v_size) {
      ...
      state->move_to_child(
          g, state->get_coloring()->lab[(best_color + (perturbe % state->get_coloring()->ptn[best_color]))]);
    }
  }
    ...
    while (state->get_coloring()->cells != g->v_size) {
      ...
      if (use_probing) {
        ...
        for (int i = 0; i < state->c->domain_size;) {
          ...
          if (static_cast<int>(candidates.size()) >= probe_limit_candidates)
            break;
          i += col_sz;
        }
        ...
        for (int i = 0; i < static_cast<int>(candidates.size()); ++i) {
          ...
          if (deviated > best_score_deviate || ((deviated == best_score_deviate) && (cells > best_score_cells)) ||
              ((deviated == best_score_deviate) && (cells == best_score_cells) && (col_sz > best_score_size))) {
            best_color = col;
        ...
        if (best_score_deviate > 0)
          --probe_limit_candidates;
      } else {
        // heuristic, try to pick "good" color
        candidates.clear();
        int best_score = -1;
      ...
      if (probe_limit_candidates < 2 || state->s_base_pos > 5)
        use_probing = false;
      ...
      int v = best_test_v;
      if (v == -1)
        v = state->get_coloring()->lab[(best_color + (perturbe % col_sz))];
...
public:
  ...
  bool check_deviation(uint64_t deviation) { return !deviation_done || map.find(deviation) != map.end(); }
};
...
class stored_leaf {
public:
  enum stored_leaf_type {
    STORE_LAB, ///< stores coloring of the leaf
  ...
  const int *get_lab_or_base() { return lab_or_base.get_array(); }

  int get_lab_or_base_size() { return lab_or_base.size(); }

  dej_nodiscard stored_leaf_type get_store_type() const { return store_type; }
...
public:
  ...
  shared_leaves() { leaf_store.reserve(20); }
  ...
  void add_leaf(uint64_t hash, coloring &c, std::vector<int> &base) {
    ...
    if (leaf_store.find(hash) != leaf_store.end())
      return;
    ...
    auto new_leaf = full_save ? new stored_leaf(c.lab, c.domain_size, type) : new stored_leaf(base, type);
...
public:
  ...
  tree_node *get_next() { return next; }
  tree_node *get_parent() { return parent; }
  void set_next(tree_node *new_next) { this->next = new_next; }
  limited_save *get_save() { return data; }

  void prune() { is_pruned = true; }
  dej_nodiscard bool get_prune() const { return is_pruned; }
  void add_hash(uint64_t add) { this->hash += add; }

  dej_nodiscard uint64_t get_hash() const { return hash; }

  void base() { is_base = true; }

  dej_nodiscard bool get_base() const { return is_base; }
...
public:
  ...
  explicit shared_tree(int domain_size) { h_bfs_top_level_orbit.initialize(domain_size); };
  ...
  std::vector<uint64_t> *get_node_invariant() { return &node_invariant; }
  ...
  void clear_leaves() { stored_leaves.clear(); }

  /**
   * @return How many leaves were stored during random search.
   */
  int stat_leaves() const { return stored_leaves.s_leaves; }
  ...
  bool reset(std::vector<int> &new_base, ir::limited_save *root, bool keep_old) {
    ...
    for (int i = 0; i < root->get_coloring()->domain_size; ++i)
      node_invariant[i] = 0;
    ...
    if (keep_old) {
      for (; keep_until < old_size && keep_until < new_size; ++keep_until) {
        if (current_base[keep_until] != new_base[keep_until])
          break;
      }
    }
  ...
  void queue_reserve(const int n) { missing_nodes.reserve(n); }

  void queue_missing_node(missing_node node) { missing_nodes.add(node); }

  bool queue_missing_node_empty() { return missing_nodes.empty(); }

  missing_node queue_missing_node_pop() { return missing_nodes.pop(); }

  void mark_first_level(markset &marks) {
    if (tree_data[1] == nullptr)
      return;
  ...
  void record_invariant(int v, uint64_t inv) { node_invariant[v] = inv; }

  void record_add_invariant(int v, uint64_t inv) { node_invariant[v] += inv; }
  ...
  dej_nodiscard int get_finished_up_to() const { return finished_up_to; }

  void set_finished_up_to(const int new_finished_up_to) { this->finished_up_to = new_finished_up_to; }

  tree_node *get_level(int level) { return tree_data[level]; }

  int get_current_level_size() { return tree_level_size[finished_up_to]; }

  int get_current_level_tracepos() { return tree_data[finished_up_to]->get_save()->get_trace_position(); }

  int get_level_size(int level) { return tree_level_size[level]; }
