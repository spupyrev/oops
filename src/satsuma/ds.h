 ...
 */
template <class T> void inline sort_t(T *arr, int sz) { std::sort(arr, arr + sz); }
...
public:
   ...
   */
  void add(T &t) { queue.emplace_back(t); }

  /**
   * Reserve space for at least \n elements.
   * @param n Space to be reserved.
   */
  void reserve(int n) { queue.reserve(n); }
 ...
 */
template <class T> class worklist_t {
private:
  /**
...
public:
  ...
  worklist_t(const worklist_t<T> &other) { copy(&other); }

  worklist_t(const worklist_t<T> &&other) { swap(other); }
   ...
   */
  inline T *last() const { return &arr[cur_pos - 1]; }

  /**
   * @return Whether `cur_pos == 0`.
   */
  dej_nodiscard bool empty() const { return cur_pos == 0; }
   ...
   */
  void set_size(const int size) { cur_pos = size; }

  /**
   * @return The current position \a cur_pos.
   */
  dej_nodiscard int size() const { return cur_pos; }

  /**
   * Sets \a cur_pos to `0`.
   */
  inline void reset() { cur_pos = 0; }
   ...
   */
  void sort() { sort_t<T>(arr, cur_pos); }

  /**
   * @return A pointer to the internal memory.
   */
  inline T *get_array() const { return arr; }
  ...
  void sort_after_map(T *map) {
    struct comparator_map {
      ...
      explicit comparator_map(T *m) { this->map = m; }

      bool operator()(const T &a, const T &b) { return map[a] < map[b]; }
    };
...
public:
   ...
   * @param size Size to allocate.
   */
  explicit workspace(int size) { allocate(size); }

  workspace(const workspace &other) { copy(other); }
   ...
   * @param size Size to allocate.
   */
  void allocate(int size) { alloc(size); }

  /**
   * Sets the entire array to 0.
   */
  inline void reset() { memset(arr, 0, arr_sz * sizeof(int)); }
   ...
   * @return A pointer to the internal memory.
   */
  dej_nodiscard inline int *get_array() const { return arr; }
 ...
 */
template <class T> class workset_t {
public:
  /**
   * Default constructor, does not allocate any memory.
   */
   ...
   * @param size Size to allocate.
   */
  explicit workset_t(int size) { initialize(size); }

  workset_t(const workset_t<T> &other) { copy(other); }

  workset_t(const workset_t<T> &&other) { swap(other); }
...
class markset {
  ...
  void full_reset() { memset(s, mark, sz * sizeof(int)); }
...
public:
  ...
  markset(const markset &other) { copy(&other); }

  markset(markset &&other) { swap(other); }
   ...
   */
  explicit markset(int size) { initialize(size); }
