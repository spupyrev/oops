...
class automorphism_workspace {
   ...
   */
  void invalidate_inverse_automorphism() { have_inverse_automorphism = false; }
...
public:
  ...
  void set_support01(const bool new_support01) {
    ...
    if (update_support_necessary)
      update_support();
  }

  bool get_support01() { return support01; }
  ...
  void write_color_diff(const int *vertex_to_col, const int *col_to_vertex) {
    color_diff_automorphism(domain_size, vertex_to_col, col_to_vertex, automorphism.get_array(), &automorphism_supp);
    invalidate_inverse_automorphism();
  }
   ...
   * @param pwr Power with which \p other is applied to this automorphism.
   */
  void apply(worklist &scratch_apply1, worklist &scratch_apply2, markset &scratch_apply3, automorphism_workspace &other,
             int pwr = 1) {
    if (!other.support01 && other.nsupp() <= domain_size / 4) {
      apply_sparse(scratch_apply1, scratch_apply2, scratch_apply3, other.p(), other.supp(), other.nsupp(), pwr);
    } else {
  ...
  void apply(worklist &scratch_apply1, worklist &scratch_apply2, markset &scratch_apply3, const int *p, int pwr = 1) {
    ...
    if (pwr <= 5) {
      if (pwr == 1)
        for (int i = 0; i < domain_size; ++i)
          automorphism[i] = p[automorphism[i]];
      else if (pwr == 2)
        for (int i = 0; i < domain_size; ++i)
          automorphism[i] = p[p[automorphism[i]]];
      else if (pwr == 3)
        for (int i = 0; i < domain_size; ++i)
          automorphism[i] = p[p[p[automorphism[i]]]];
      else if (pwr == 4)
        for (int i = 0; i < domain_size; ++i)
          automorphism[i] = p[p[p[p[automorphism[i]]]]];
      else if (pwr == 5)
        for (int i = 0; i < domain_size; ++i)
          automorphism[i] = p[p[p[p[p[automorphism[i]]]]]];
    } else if (pwr <= 19) {
      ...
      if (pwr == 1)
        ...
        for (int i = 0; i < domain_size; ++i)
          automorphism[i] = scratch_apply1[automorphism[i]];
      else if (pwr == 4)
        for (int i = 0; i < domain_size; ++i)
          automorphism[i] = p[scratch_apply1[automorphism[i]]];
      else if (pwr == 5)
        for (int i = 0; i < domain_size; ++i)
          automorphism[i] = p[p[scratch_apply1[automorphism[i]]]];
    } else {
      // 1 cycle at a time

      scratch_apply3.reset();
      for (int i = 0; i < domain_size; ++i) {
        if (scratch_apply3.get(i))
          continue;
        if (p[i] == i)
          scratch_apply2[i] = i;
        else {
      ...
      for (int i = 0; i < domain_size; ++i)
        automorphism[i] = scratch_apply2[automorphism[i]];
      scratch_apply3.reset();
    }
  ...
  void apply_sparse(worklist &scratch_apply1, worklist &scratch_apply2, markset &scratch_apply3, const int *p,
                    const int *support, const int nsupport, int pwr = 1) {
    if (pwr == 0)
      return;
   ...
   */
  void write_singleton(const std::vector<int> *singletons1, const std::vector<int> *singletons2, const int pos_start,
                       const int pos_end) {
  ...
  void cycle_completion(markset &scratch_set) {
    ...
    for (int i = 0; i < automorphism_supp.size(); ++i) {
      ...
      if (scratch_set.get(v))
        continue;
      int v_next = automorphism[v];
      while (v_next != v) {
        if (scratch_set.get(v_next))
          break;
   ...
   */
  dej_nodiscard int *p() const { return automorphism.get_array(); }

  /**
   * @return Integer array which contains all vertices in the support of the contained automorphism.
   */
  dej_nodiscard int *supp() const { return automorphism_supp.get_array(); }
   ...
   * trivial.
   */
  dej_nodiscard int nsupp() const { return automorphism_supp.cur_pos; }
};
...
public:
   ...
   */
  dej_nodiscard bool represents_orbit(const int v) const { return v == map_arr[v]; }
   ...
   * @param domain_size size of the underlying domain
   */
  explicit orbit(int domain_size) { initialize(domain_size); }
...
public:
  void load(int *automorphism) { loaded_automorphism = automorphism; }

  int *p() { return loaded_automorphism; }
};
...
public:
  enum stored_automorphism_type {
    STORE_DENSE, ///< stored densely, in size O(\a domain_size)
...
public:
   ...
   */
  stored_automorphism_type get_store_type() { return store_type; }
  ...
  void load(automorphism_workspace &automorphism) {
    if (store_type == STORE_SPARSE) {
      ...
      for (int i = 0; i < domain_size; ++i) {
        ...
        if (v_from != v_to)
          automorphism.write_single_map(v_from, v_to);
      }
    }
  }
...
public:
   ...
   */
  void initialize(int new_domain_size) { this->domain_size = new_domain_size; }
  ...
  int add_generator(schreier_workspace &w, automorphism_workspace &automorphism) {
    ...
    s_stored_sparse +=
        (generators[num]->get_store_type() == stored_automorphism::stored_automorphism_type::STORE_SPARSE);
   ...
   */
  dej_nodiscard stored_automorphism *get_generator(const size_t num) const { return generators[num]; }

  stored_automorphism *operator[](const size_t num) const { return get_generator(num); }

  /**
   * @return number of stored generators
   */
  dej_nodiscard int size() const { return static_cast<int>(generators.size()); }

  /**
   * @return memory size of stored generators
   */
  dej_nodiscard long get_support() const { return support_size; }
...
class shared_transversal {
   ...
   */
  static void apply_perm(schreier_workspace &w, automorphism_workspace &automorphism, generating_set &generators,
                         const int gen_num, const int pwr) {
...
public:
   ...
   */
  dej_nodiscard int size() const { return (int)fixed_orbit.size(); }

  void set_size_upper_bound(const int new_sz_upb) { sz_upb = new_sz_upb; }

  dej_nodiscard int get_size_upper_bound() const { return sz_upb; }
  /**
   ...
   */
  dej_nodiscard bool is_finished() const { return finished; }

  /**
   * @return Point fixed by this transversal.
   */
  dej_nodiscard int get_fixed_point() const { return fixed; }

  dej_nodiscard const std::vector<int> &get_fixed_orbit() { return fixed_orbit; }

  dej_nodiscard const std::vector<int> &get_generators() { return fixed_orbit_to_perm; }
  ...
  bool extend_with_automorphism(schreier_workspace &w, generating_set &generators,
    ...
    if (finished)
      return false; // Already finished transversal? Nothing to do, then!
    ...
    for (int i = 0; i < static_cast<int>(fixed_orbit.size()); ++i) {
      ...
      while (!w.scratch1.get(j)) {
        ...
        if (gen_num == -1)
          gen_num = generators.add_generator(w, automorphism);
...
public:
   ...
   */
  dej_nodiscard int s_sparsegen() const { return generators.s_stored_sparse; }

  /**
   * @return Number of stored generators using a dense data structure.
   */
  dej_nodiscard int s_densegen() const { return generators.s_stored_dense; }
  ...
  void set_base(schreier_workspace &w, automorphism_workspace &automorphism, random_source &rng,
    ...
    for (; keep_until < old_size && keep_until < new_size; ++keep_until) {
      if (transversals[keep_until].get_fixed_point() != new_base[keep_until])
        break;
    }

    if (keep_until == new_size)
      return;
    ...
    if (resift_generators) {
      for (int gen_id : stabilized_generators_copy)
        sift_generator(w, automorphism, gen_id, true);
    }
   ...
   */
  bool reset(int new_domain_size, schreier_workspace &w, std::vector<int> &new_base, std::vector<int> &new_base_sizes,
             const int stop, bool keep_old, bool remove_generators, std::vector<int> &global_fixed_points) {
    const int old_size = static_cast<int>(transversals.size());
    ...
    if (keep_until == new_size && new_size == old_size)
      return false;
   ...
   */
  dej_nodiscard int base_point(int pos) const { return transversals[pos].get_fixed_point(); }

  /**
   * @return Size of base of this Schreier structure.
   */
  dej_nodiscard int base_size() const { return static_cast<int>(transversals.size()); }
  ...
  const std::vector<int> &get_stabilized_generators() { return stabilized_generators; }

  dej_nodiscard int get_fixed_orbit_size(const int base_pos) {
    if (base_pos >= static_cast<int>(transversals.size()))
      return 0;
   ...
   */
  dej_nodiscard bool is_finished(const int base_pos) const { return transversals[base_pos].is_finished(); }

  /**
   * @return memory size of stored generators
   */
  dej_nodiscard long get_support() const { return generators.get_support(); }
  ...
  bool sift(schreier_workspace &w, automorphism_workspace &automorphism, bool uniform = false,
    ...
    if (uniform)
      record_sift_result(changed); // uniform automorphisms count towards abort criterion
   ...
   */
  void reset_probabilistic_criterion() { s_consecutive_success = 0; }

  /**
   * @return Whether the probabilistic abort criterion allows termination or not.
   */
  dej_nodiscard bool probabilistic_abort_criterion() const { return (s_consecutive_success > h_error_bound); }

  /**
   * @return Whether the deterministic abort criterion allows termination or not.
   */
  dej_nodiscard bool deterministic_abort_criterion() const { return (finished_up_to_level() + 1 == base_size()); }
   ...
   */
  dej_nodiscard int finished_up_to_level() const { return finished_up_to; }
  ...
  void compute_group_size() {
    ...
    for (auto &transversal : transversals)
      s_grp_sz.multiply(transversal.size());
  }
};
...
public:
   ...
   * @return Number of stored generators using a sparse data structure.
   */
  dej_nodiscard int get_number_of_generators() const { return schreier.s_sparsegen() + schreier.s_densegen(); }
   ...
   */
  void get_generator(int i, automorphism_workspace &automorphism) { schreier.load_generator(automorphism, i); }
  ...
  explicit random_schreier(int domain_size, int error = 10, bool true_random = false, int seed = 0)
      : ws_auto(domain_size), ws_schreier(domain_size), rng(true_random, seed), auxiliary_set(domain_size) {
    h_error_bound = error;
  ...
  void set_base(std::vector<int> &new_base, bool resift_generators = false) {
    if (!init) {
      ...
      for (int i = 0; i < static_cast<int>(new_base.size()); ++i)
        base_sizes.push_back(INT32_MAX);
      schreier.initialize(h_domain_size, new_base, base_sizes);
      init = true;
    } else
      schreier.set_base(ws_schreier, ws_auto, rng, new_base, h_error_bound, resift_generators);
    sift_random();
  }
   ...
   */
  void sift_random() { schreier.sift_random(ws_schreier, ws_auto, rng, h_error_bound); }

  /**
   * @return Size of base of this Schreier structure.
   */
  dej_nodiscard int base_size() const { return schreier.base_size(); }

  /**
   * @param pos Position in base.
   * @return Vertex fixed at position \p pos in base.
   */
  dej_nodiscard int get_fixed_point(int pos) const { return schreier.base_point(pos); }
   ...
   * @return Bool indicating whether \p v is contained in the transversal at position \p s_base_pos.
   */
  bool is_in_fixed_orbit(const int base_pos, const int v) { return schreier.is_in_fixed_orbit(base_pos, v); }
   ...
   */
  dej_nodiscard int get_fixed_orbit_size(const int base_pos) { return schreier.get_fixed_orbit_size(base_pos); }
   ...
   */
  const std::vector<int> &get_fixed_orbit(const int base_pos) { return schreier.get_fixed_orbit(base_pos); }
...
public:
  ...
  void determine_compression_map(coloring &c, std::vector<int> &base, int stop) {
    ...
    s_compression_ratio = 1.0;
    if (domain_size > 0)
      s_compression_ratio = 1.0 * s_vertices_active / domain_size;
  }

  /**
   * @return how large the compressed domain is
   */
  dej_nodiscard int compressed_domain_size() const { return s_vertices_active; }

  /**
   * @return how large the original domain is
   */
  dej_nodiscard int decompressed_domain_size() const { return domain_size; }
...
public:
   ...
   * @return Number of stored generators using a sparse data structure.
   */
  dej_nodiscard int s_sparsegen() const { return internal_schreier.s_sparsegen(); }

  /**
   * @return Number of stored generators using a dense data structure.
   */
  dej_nodiscard int s_densegen() const { return internal_schreier.s_densegen(); }
   ...
   * @param keep_old If true, attempt to keep parts of the base that is already stored.
   */
  bool reset(domain_compressor *new_compressor, int new_domain_size, schreier_workspace &w, std::vector<int> &new_base,
             std::vector<int> &new_base_sizes, const int stop, bool keep_old, std::vector<int> &global_fixed_points) {
    ...
    if (compressor != nullptr && compressor->s_compression_ratio > h_min_compression_ratio)
      compressor = nullptr;
    ...
    if (compressor != nullptr) {
      ...
      return internal_schreier.reset(new_compressor->compressed_domain_size(), w, new_basec, new_base_sizes, stop,
                                     keep_old, remove_generators, global_fixed_points);
    } else {
      // we are not using compression, so simply use the Schreier structure normally
      s_compression_ratio = 1.0;
      return internal_schreier.reset(new_domain_size, w, new_base, new_base_sizes, stop, keep_old, remove_generators,
                                     global_fixed_points);
    }
  }
   ...
   * @return Size of base of this Schreier structure.
   */
  dej_nodiscard int base_size() const { return internal_schreier.base_size(); }
   ...
   * Reset the probabilistic abort criterion.
   */
  void reset_probabilistic_criterion() { internal_schreier.reset_probabilistic_criterion(); }

  /**
   * @return memory size of stored generators
   */
  dej_nodiscard long get_support() const { return internal_schreier.get_support(); }

  /**
   * @return Whether the probabilistic abort criterion allows termination or not.
   */
  dej_nodiscard bool probabilistic_abort_criterion() const { return internal_schreier.probabilistic_abort_criterion(); }

  /**
   * @return Whether the deterministic abort criterion allows termination or not.
   */
  dej_nodiscard bool deterministic_abort_criterion() const { return internal_schreier.deterministic_abort_criterion(); }

  /**
   * @return Whether any abort criterion allows termination or not.
   */
  dej_nodiscard bool any_abort_criterion() const {
    return internal_schreier.probabilistic_abort_criterion() || internal_schreier.deterministic_abort_criterion();
  }

  void set_error_bound(int error_bound) { internal_schreier.h_error_bound = error_bound; }

  dej_nodiscard int get_consecutive_success() const { return internal_schreier.s_consecutive_success; }

  dej_nodiscard big_number get_group_size() const { return internal_schreier.s_grp_sz; }

  /**
   * @return Level up to which Schreier structure is guaranteed to be complete according to given upper bounds.
   *     -1 indicates no level has been finished.
   */
  dej_nodiscard int finished_up_to_level() const { return internal_schreier.finished_up_to_level(); }

  /**
   * @return Size of group described by this Schreier structure.
   */
  void compute_group_size() { internal_schreier.compute_group_size(); }
};
} // namespace groups
} // namespace dejavu
