...
public:
  ...
  int paired_recurse_to_equal_leaf(sgraph *g, ir::controller &state_left, ir::controller &state_right,
    ...
    if (!recurse) {
      ...
      if (!is_diffed_pre || state_right.get_diff_diverge())
        return 2;
    }
    ...
    while (state_right.c->cells < g->v_size) {
      ...
      if (col_sz_right < 2 || col_sz_left != col_sz_right)
        return 0;
      ...
      // invariant differs
      if (state_right.T->get_hash() != state_left.T->get_hash())
        return 0;
  ...
  int do_paired_dfs(dejavu_hook *hook, sgraph *g, ir::controller &state_left, ir::controller &state_right,
    ...
    if (state_right.s_base_pos == 1)
      return do_simple_dfs(hook, g, state_right, computed_orbits, prune);

    if (h_recent_cost_snapshot_limit < 0)
      return state_right.s_base_pos;
    ...
    while ((recent_cost_snapshot < h_recent_cost_snapshot_limit || state_right.s_base_pos <= 1) &&
      ...
      if ((state_right.s_base_pos & 0x00000FFF) == 0x00000FFD)
        ws_printer.progress_current_method(
            "dfs", "base_pos", static_cast<double>(state_right.compare_base->size()) - state_right.s_base_pos,
      ...
      for (int i = 0; i < col_sz; ++i) {
        ...
        if (orbs.are_in_same_orbit(ind_v, base_vertex))
          continue;
        // if(!orbs.represents_orbit(ind_v)) continue;
        if (!orbs.represents_orbit(ind_v))
          ind_v = orbs.find_orbit(ind_v);
        if (orbit_handled.get(ind_v))
          continue;
        ...
        dej_assert(state_right.c->vertex_to_col[ind_v] == state_right.leaf_color.vertex_to_col[base_vertex]);
        ...
        if (!pruned) {
          ...
          ws_automorphism.write_singleton(state_right.compare_singletons, &state_right.singletons, wr_pos_st,
                                          wr_pos_end);
          if (g->v_size != state_right.c->cells)
            ws_automorphism.cycle_completion(workspace);
        ...
        if (found_auto) {
          ...
          dej_assert(ws_automorphism.p()[ind_v] == base_vertex || ws_automorphism.p()[base_vertex] == ind_v);
          orbs.add_automorphism_to_orbit(ws_automorphism);
          if (hook)
            (*hook)(g->v_size, ws_automorphism.p(), ws_automorphism.nsupp(), ws_automorphism.supp());
        }
        ws_automorphism.reset();
        ...
        if ((!found_auto && !pruned) || ((4 * prune_cost_snapshot > cost_snapshot) && (prev_base_pos > 0)) ||
            (pruned && !prune)) {
          fail = true;
        ...
        if (orbs.orbit_size(base_vertex) == col_sz)
          break;
      }
