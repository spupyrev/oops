...
using satsuma::graph_to_sat;
using satsuma::sat_to_graph;
...
public:
  ...
  void mark_literal_uses() {
    for (int i = 0; i < n_clauses(); ++i) {
      if (clause_satisfied.get(i))
        continue;
      for (int j = 0; j < clause_size(i); ++j) {
        const int lit = literal_at_clause_pos(i, j);
        if (assigned(lit) == 0)
          found_literal.set(sat_to_graph(lit));
      }
    }
  }

  bool is_literal_marked_used(int lit) { return found_literal.get(sat_to_graph(lit)); }
  ...
  bool is_satisfied(int clause) { return clause_satisfied.get(clause); }
  ...
  void assign_literal(int literal) {
    ...
    for (auto c : literal_used_list[sat_to_graph(literal)])
      update_satisfied(c);
  ...
  int assigned(int literal) { return assignment[sat_to_graph(literal)]; }

  void update_satisfied(int clause_number) { clause_satisfied.set(clause_number); }

  void add_watch(int variable, int watch_clause) { variable_watches_clause[abs(variable)].push_back(watch_clause); }
  ...
  void update_watches(int clause_number, int from_variable, int from_pos) {
    ...
    // std::clog << clause_number << ":" << abs(clauses[watch1]) << ", " <<  abs(clauses[watch2]) << ", " <<
    // from_variable << std::endl;
  ...
  int clause_size(int c) { return clauses_pt[c].second - clauses_pt[c].first; }

  int literal_at_clause_pos(int c, int i) { return clauses[clauses_pt[c].first + i]; }

  int n_total_clause_size() { return clauses.size(); }

  int n_len() { return clauses.size(); }

  int n_clauses() { return clauses_pt.size(); }

  int n_variables() { return number_of_variables; }
