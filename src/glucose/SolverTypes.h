...
// Useful special constants.
constexpr Lit lit_Undef = {-2};
constexpr Lit lit_Error = {-1};
...
#define l_True (Simp21::lbool((uint8_t)0)) 
#define l_False (Simp21::lbool((uint8_t)1))
#define l_Undef (Simp21::lbool((uint8_t)2))
...
public:
  ...
  template <class Lits> CRef alloc(const Lits &ps, bool learnt = false) {
    ...
    const int extras = learnt ? 2 : (int)extra_clause_field;
  ...
  void free(CRef cid) {
    ...
    const int extras = c.learnt() ? 2 : (int)c.has_extra();
