...
\begin{proof}[Proposed joint computational proof]
    ...
    If neither reduction is available, the parent $G$ itself is retained and
    checked directly for 1-planarity.  Consequently the computation needs
    no three-edge-star verification at order 26 and no flexibility
    assumption beyond the ordinary Claim~3 statement.  The global
...
hours; that route was abandoned.  In a fixed 10,000-parent sample, the joint
preparation above used a completed order-24 certificate for 4,465 parents,
selected a contraction core for 5,535, and left no direct residual.  The
4,080 distinct selected codes include the Claim~3 cores that will be removed
by an exact set comparison.  Preparing the sample took 19.73 seconds in one
process.  A separate 884-core stress sample of the extra all-attachment
requirement took 33.15 seconds, and all records passed.
...
17.1--26.8 hour sampling envelope.  The smaller Claim~3 girth-six and
Claim~4 checks add approximately 2.7--5.4 hours.  Combining these figures
with the observed 15--24 hour preparation envelope gives a central
37--53 hour projection and a 35--56 hour sampling envelope.  The final
launch decision uses the exact globally merged count and a bounded timing
...
\begin{table}[!ht]
    ...
    \begin{tabular}{@{}lrrc@{}}
        ...
        \midrule
        remaining Claims 3--4 & & 35--56 hours${}^\dagger$ (projected) & no \\
        \bottomrule
    \end{tabular}
    \par\smallskip
    {\footnotesize $^\ast$Projected number of distinct Claim 3 cores.
    $^\dagger$Including the observed 15--24 hour preparation envelope, but
    before the exact global set difference.\par}
\end{table}
