...
\emph{order} of a graph $G$ is $|V(G)|$.  A graph is \emph{cubic} if every
vertex has degree three, and it is \emph{subcubic} if every vertex has degree
at most three.  A connected graph is \emph{biconnected} if it has no cut
vertex.  A graph is \emph{planar} if it has a drawing without crossings.  The
\emph{girth} of a graph containing a cycle is the length of its shortest
cycle.
...
\begin{lemma}[Drawing criterion for flexibility]\label{lem:drawing-coverage}
    Let $\mathcal D$ be a finite collection of 1-planar drawings of a graph
...
The computation constructs a collection $\mathcal D$ as follows.  Beginning
with one verified drawing, it searches for a set $F$, $|F|\le k$, that
intersects $X_D$ for every $D\in\mathcal D$.  If no such set exists,
\cref{lem:drawing-coverage} applies.  Otherwise it searches for a 1-planar
drawing in which every edge of $F$ is uncrossed.  Each resulting drawing is
verified independently before it is added to $\mathcal D$.  The procedure
terminates because the new crossed-edge set is disjoint from the set $F$
found in that iteration.  Consequently, the same $F$ cannot be returned in a
later iteration, and there are only finitely many subsets of $E(G)$ of size at
most $k$.

Verification of a proposed drawing requires only ordinary planarity of
the graph obtained by replacing every proposed crossing pair by a degree-four
vertex.  At such a vertex, alternate edge ends are redrawn as a proper
crossing; consecutive ends of the same edge are instead smoothed without a
crossing.  Proposed crossing pairs form a matching, so this produces a
1-planar drawing whose actually crossed edges are a subset of the recorded
set $X_D$.  Accordingly, \cref{lem:drawing-coverage} permits $X_D$ to be a
superset of the edges that cross in the resulting drawing.

The following local observation justifies the crossing elimination used in
the constrained drawing searches.
...
\begin{lemma}[4-cycle reduction through order 24]\label{lem:order24-reduction}
    ...
    every cubic graph of order 24 and girth at least five is $2$-flexible.
    Then every cubic graph of order at most 24 is $2$-flexible.
\end{lemma}
...
\begin{proof}
    ...
    most two edges.  If $G$ has no 4-cycle, the second hypothesis applies.
    Otherwise choose a 4-cycle $C$.
...
triangle-free cubic graphs of order at most 22 and $2$-flexibility for cubic
graphs of order 24 and girth at least five.
...
\begin{proof}[Proof (computational)]
    ...
    disk with alternating incidences at each crossing vertex.  This finite
    check finds a valid drawing for every boundary order; insert the
...
would create a cycle of length at most five.  Choose any 6-cycle, contract
it, and require the four attachment edges specified in
\cref{lem:6-cycle-expansion} to be uncrossed.  A graph with no 6-cycle has
girth at least seven.  A 1-planar drawing of each such graph is computed and
independently validated.  There are 21 such graphs at order 28; an independent
enumeration of the girth-at-least-seven family confirms this number.  No
simple planar cubic graph has girth at least six: Euler's inequality
...
is 1-planar.  Direct recognition of all graphs at the two largest orders is
not practical: there are $2{,}094{,}480{,}864$ connected cubic graphs of order 26
and $40{,}497{,}138{,}011$ of order 28.  Even at ten milliseconds per graph, the
order-28 computation alone would require more than twelve CPU-years.

We replace direct recognition by five computer-assisted statements together
with the preceding reductions.  Triangles and 4-cycles reduce to smaller
graphs with prescribed uncrossed edges.  A 5-cycle at order 28 is replaced
by a three-vertex path and expanded using \cref{lem:5-cycle-expansion}; 6-cycles
are treated by \cref{lem:6-cycle-expansion}.  It therefore suffices to
enumerate the graph classes stated below.

For completeness, we briefly describe how the computational claims are
certified.  Each graph class is generated up to isomorphism.  A proposed
drawing is represented by a matching of pairs of edges that are designated to
cross.  Replacing each designated crossing by a vertex of degree four produces
a planar graph.  In a planar embedding of this graph, alternating incidences
at the new vertex yield the designated crossing, whereas consecutive
incidences can be smoothed without a crossing.  The matching condition implies
that no edge is crossed more than once.  For the flexibility statements, the
computation constructs a collection of verified drawings satisfying the
hypothesis of \cref{lem:drawing-coverage}.  Thus every computational conclusion
follows from an exhaustive enumeration and explicitly verified 1-planar
drawings.
...
\begin{claim}\label{clm:flexible22}
    ...
    $n \in \{20,22\}$ is also $3$-flexible.
\end{claim}

\begin{proof}[Proof (computational)]
    Exhaustive generation up to isomorphism produces $1{,}575{,}835$ graphs in
    these families.  Every planar graph is $3$-flexible.  For each nonplanar graph
    $G$, the computation constructs a finite collection $\mathcal D$ of
    verified 1-planar drawings such that no set of at most three edges of $G$
    intersects $X_D$ for every $D\in\mathcal D$.  The claim follows from
    \cref{lem:drawing-coverage}.
\end{proof}

\begin{claim}\label{clm:flexible24}
    Every connected cubic graph with $n=24$ and girth $\geq 5$ is
    $2$-flexible.
\end{claim}

\begin{proof}[Proof (computational)]
    There are $1{,}620{,}479$ isomorphism classes in this family.  For each
    nonplanar graph $G$, the computation constructs a finite collection
    $\mathcal D$ of verified 1-planar drawings such that no set of at most two
    edges of $G$ intersects $X_D$ for every $D\in\mathcal D$.  Hence $G$ is
    $2$-flexible by \cref{lem:drawing-coverage}.
\end{proof}
...
\begin{proof}[Proof (computational)]
    ...
    nonplanar graph and every vertex $v$, the computation obtains and verifies
    a 1-planar drawing in which the three edges incident with $v$ are
    uncrossed.  One drawing may satisfy this requirement for several
    vertices.  Planar graphs satisfy the requirement in a planar drawing.
\end{proof}
...
\begin{proof}[Proof (computational)]
    There are $652{,}159{,}389$ isomorphism classes.  For every nonplanar
    graph, the computation first searches for a 5-cycle and a cyclic choice
    of its middle attachment such that the replacement in
    \cref{lem:5-cycle-expansion} is a simple connected cubic graph $H$ with 26
    vertices and girth at least five.  When such a replacement exists,
    \cref{clm:stars26} supplies a drawing of $H$ in which the three edges
    incident with the middle vertex of the replacement path are uncrossed;
    \cref{lem:5-cycle-expansion} then gives a drawing of the original graph.
    If no such replacement exists, the computation obtains and independently
    verifies a 1-planar drawing of the original graph.
\end{proof}
...
\begin{proof}[Proof (computational)]
    There are $4{,}624{,}501$ isomorphism classes in this family.  A 6-cycle
    occurs in $4{,}624{,}480$ of them.  For each of these graphs, contracting a
    6-cycle gives a graph with a verified 1-planar drawing in which the four
    attachment edges specified in \cref{lem:6-cycle-expansion} are uncrossed.
    Consequently, the original graph is 1-planar by
    \cref{lem:6-cycle-expansion}.  The remaining 21
    graphs have girth at least seven, and a verified 1-planar drawing was
    obtained for each of them.
\end{proof}

The projected running times for the five computations are summarized in
\cref{tab:cubic28-runtime}.  They are extrapolated from computations on
representative subsets and on complete subsets of the enumerations.
The one-core column gives the projected aggregate processor time; the
32-core column assumes an even distribution of the work.  Time required to
enumerate the graph families is not included.

\begin{table}[!ht]
    \small
    \centering
    \caption{Projected running times for the computational claims.}
    \label{tab:cubic28-runtime}
    \begin{tabular}{@{}lrr@{}}
        \toprule
        computation & one core & 32 cores \\
        \midrule
        \cref{clm:flexible22} & 4.6 days & 3.5 hours \\
        \cref{clm:flexible24} & 6.7 days & 5.0 hours \\
        \cref{clm:stars26}    & 279 days & 8.7 days \\
        \cref{clm:girth5-28}  & 6.3 days & 4.7 hours \\
        \cref{clm:girth6-28}  & 12.3 days & 9.2 hours \\
        \midrule
        total & 309 days & 9.7 days \\
        \bottomrule
    \end{tabular}
\end{table}

The five claims reduce the proof to the structural arguments established
...
\begin{proof}
    By \cref{lem:flexibility-core,clm:flexible22}, every subcubic graph of
    order at most 22 is $3$-flexible.  Applying
    \cref{lem:order24-reduction,clm:flexible24} shows that every cubic graph
    of order at most 24 is $2$-flexible and hence 1-planar.

    A disconnected cubic graph of order 26 or 28 has only components of order
    at most 24, whose drawings can be placed disjointly.  It therefore remains
    to consider connected graphs at these two orders.

    Consider order 26.  A triangle reduces to a smaller subcubic graph.  In a
    triangle-free graph containing a 4-cycle, fix any prescribed edge and
    split that cycle.  If the prescribed edge lies on the cycle, choose the
    split that creates it inside an endpoint disk; otherwise retain its image.
    Together with the new middle edge this gives at most two prescribed edges
    in the order-24 graph, covered by its $2$-flexibility.  A
    triangle-free graph with no 4-cycle has girth at least five and is
    covered by \cref{clm:stars26}.  The property in \cref{clm:stars26}
    implies $1$-flexibility, since each prescribed edge is incident with a
    vertex whose entire three-edge star can be kept uncrossed.  Hence every
    order-26 cubic graph is $1$-flexible and therefore 1-planar.

    Finally consider order 28.  Triangles reduce as before.  In a triangle-free
    graph containing a 4-cycle, split that cycle once and use the
    established order-26 $1$-flexibility to keep the new middle edge
    uncrossed.  A graph with neither a triangle nor a 4-cycle has girth at
    least five.  The case of girth exactly five is covered by
    \cref{clm:girth5-28}, and the case of girth at least six is covered by
    \cref{clm:girth6-28}.  These cases exhaust all simple cubic graphs of
    order at most 28.
\end{proof}
