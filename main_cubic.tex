...
\begin{proof}[Proposed computational proof]
    ...
    5-cycle-to-path reductions $(H,b)$ defined in
    ...
    If none of these certificates exists, the preparation retains the
    lexicographically least canonically labeled reduction $(H,b)$.  The
    computation then finds a drawing of $H$ in which the three edges incident
    with $b$ are uncrossed, and \cref{lem:5-cycle-path-expansion} restores
    $G$.  Thus every input obtains a direct, degree-six, degree-seven, or
    marked-path certificate, without using Claim~3.  The global preparation
    and verification have not yet been completed, so this proposed
...
\cref{tab:cubic28-runtime}.  Claims 1, 2, 3, and 5 are completed computations.
The earlier use of marked-path certificates for every input to
\cref{clm:girth5-28} produced
$38{,}120{,}815$ groups whose measured verification would take 392--470
hours; using them only as a fallback avoids that cost.
...
The earlier Claim~4 estimate used a 60,000-parent structural sample in which
every graph had a direct, degree-six, or degree-seven certificate.  The
production preparation found graphs with none of these three certificates,
so that estimate is withdrawn.  On a 401-parent prefix, four graphs required
the marked-path fallback; their four distinct reductions were verified in
104 milliseconds.  The complete run must measure the total number of
fallback records before a new running-time estimate is stated.
...
\begin{table}[!ht]
    ...
    \begin{tabular}{@{}lrrc@{}}
        ...
        \cref{clm:girth5-28}  & not yet measured & not yet measured & no \\
        \cref{clm:girth6-28}  & $4{,}624{,}501$ & 3 days (measured) & yes \\
        \midrule
        remaining Claim 4 & & not yet measured & no \\
        \bottomrule
    \end{tabular}
    \par\smallskip
...
Thus the present computations do not yet establish
\cref{clm:girth5-28}, and the theorem below remains conditional on completing
or replacing that computation.
