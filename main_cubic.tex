...
\begin{lemma}[5-cycle star expansion]\label{lem:5-cycle-star-expansion}
    Let $C=(v_0,\ldots,v_4)$ be a chordless 5-cycle in a cubic graph $G$,
    and let $x_i$ be the neighbor of $v_i$ outside $C$.  Suppose that the
    ...
    \begin{enumerate}[(i)]
        ...
        are uncrossed.
        \item Let $u\notin V(C)$, and choose two consecutive attachments
        $s_j,s_{j+1}$ neither of which is incident with $u$.  If $J$ has a
        1-planar drawing in which $s_j,s_{j+1}$ and the three edges incident
        with $u$ are uncrossed, then $G$ has a 1-planar drawing in which the
        three edges incident with $u$ are uncrossed.
    \end{enumerate}
\end{lemma}
...
\begin{proof}[Proof (computational)]
    ...
    any chosen consecutive pair $s_j,s_{j+1}$ to this pair.  If
    $u=x_i$, choose the pair to avoid $s_i$; the local drawing then does not
    cross any edge incident with $u$.  This proves (ii).
...
\begin{claim}\label{clm:stars26}
    For every biconnected, nonplanar cubic graph $G$ with $n=26$ and girth
    $\geq 5$, and every vertex $v\in V(G)$, there is a 1-planar drawing of $G$
    in which all three edges incident with $v$ are uncrossed.
\end{claim}

\begin{proof}[Proof (computational)]
    First suppose that $G$ contains a 5-cycle $C$.  Contract $C$ to one
    vertex to obtain a graph $J$ of order 22, and retain the notation
    $s_0,\ldots,s_4$ from \cref{lem:5-cycle-star-expansion}.  The computation
    considered the following 22 sets of edges of $J$:
    \begin{enumerate}[(i)]
        \item the set $\{s_0,\ldots,s_4\}$; and
        \item for every vertex $u\notin V(C)$, the three edges incident with
        $u$ together with two consecutive attachments not incident with $u$.
    \end{enumerate}
    For every such set $F$, it found a crossing pattern $M$ such that $J_M$
    is planar and $F\cap\operatorname{supp}(M)=\varnothing$.  Applying
    \cref{lem:5-cycle-star-expansion} to the first set gives the required
    drawing for every vertex of $C$; applying it to the corresponding set in
    (ii) gives the required drawing for every other vertex.

    If $G$ has no 5-cycle, then its girth is at least six.  For every vertex
    $v\in V(G)$, the computation directly found a crossing pattern $M$ such
    that $G_M$ is planar and no edge incident with $v$ belongs to
    $\operatorname{supp}(M)$.
\end{proof}
...
\begin{proof}[Proof (computational)]
    For every graph $G$ in this family, the computation established one of the
    ...
    \begin{enumerate}[(i)]
        ...
        $n=26$ and girth at least five, and $H$ is either planar or
        biconnected.
        \item There is a crossing pattern $M$ in $G$ such that $G_M$ is
        planar.
    \end{enumerate}
    In the first case, if $H$ is planar, its planar drawing leaves every edge
    uncrossed.  Otherwise $H$ is biconnected and nonplanar, so
    \cref{clm:stars26} gives a 1-planar drawing of $H$ in which the three edges
    incident with $b$ are uncrossed.  In either case,
...
\cref{tab:cubic28-runtime}.  The times for
\cref{clm:flexible22,clm:flexible24} were measured in the complete
verification; the remaining times are projected from computations on subsets
of the enumerations.  All times are wall-clock times for 48 cores.  Time
required to enumerate the graph families is not included.
...
\begin{table}[!ht]
    ...
    \begin{tabular}{@{}lrrc@{}}
        ...
        \cref{clm:stars26}    & $31{,}478{,}584^\ast$ & 33 days (preliminary) & no \\
        \cref{clm:girth5-28}  & $652{,}159{,}389^\ast$ & 15 hours (projected) & no \\
        \cref{clm:girth6-28}  & $4{,}624{,}501$ & 20 hours (projected) & no \\
        \midrule
        total & $691{,}458{,}779^\ast$ & 37 days (preliminary) & no \\
        \bottomrule
    \end{tabular}
    \par\smallskip
    {\footnotesize $^\ast$Connected-family count; replace with the
    biconnected, nonplanar count after filtering.\par}
\end{table}
