...
\begin{proof}[Proposed computational proof]
    ...
    1-planarity.  A fixed time limit is therefore used only to divide the
    exhaustive computation: for each unfinished reduction, every original
    graph assigned to that reduction is tested directly for 1-planarity.
    Consequently, each graph is covered either by one of the preceding
    expansion arguments or by a direct checked drawing; no conclusion is
    drawn from reaching the time limit.  The direct checks have not yet been
    completed, so this proposed computation does not yet prove the claim.
\end{proof}
...
so that estimate is withdrawn.  The first solver pass also retained about
70,000 records at its earlier 120-second limit.  The final computation uses a
60-second limit and checks every original graph represented by an unfinished
record directly.  The number and total running time of these direct checks
