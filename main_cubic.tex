...
\begin{proof}[Proof (computational)]
    For every graph $G$ in this family and every choice of a 5-cycle and its
    middle attachment, form the simple connected cubic graph $H$ and the
    vertex $b$ from \cref{lem:5-cycle-expansion}.  Girth five makes the five
    outside neighbors distinct, so every such reduction is valid.
    Canonically label $H$ and minimize first the code of $H$, then the
    canonical label of $b$.  Identical reduced graphs are grouped, retaining
    the set of all vertices marked in this way.

    For every grouped graph $H$ and every one of its marked vertices $b$, the
    computation found a crossing pattern whose planarization is planar and
    whose support avoids all three edges incident with $b$.  Hence $H$ has
    the drawing required by \cref{lem:5-cycle-expansion}, and the
    corresponding parent $G$ is 1-planar.  This computation uses only the
    marked reductions arising from the enumerated order-28 graphs; it invokes
    no flexibility statement about all order-26 graphs.
\end{proof}
...
The two remaining computations are projected from bounded, single-process
samples and a 48-core server pilot.  The pilot measured $269$ milliseconds
per partially merged marked group for \cref{clm:girth5-28}, or 74.7 CPU-hours
per million groups.  The machine's measured effective speedup is 25--30 with
48 processes, giving 2.99--2.49 wall-clock hours per million groups.  The
earlier estimate of 29 million groups was obtained from only 50
reverse-expanded reduced graphs and is not a reliable estimate of the global
quotient.  Even at that estimate, verification alone takes 72.2 hours at
30-fold speedup.  The final group count and marker distribution are not yet
known, so the current computation has no justified three-day bound.
...
\begin{table}[!ht]
    ...
    \begin{tabular}{@{}lrrc@{}}
        ...
        \cref{clm:flexible26} & $2.8$ million${}^\ast$ & 3 hours (projected) & no \\
        \cref{clm:girth5-28}  & $N_4$ million${}^\dagger$ & $2.49N_4$--$2.99N_4$ hours${}^\ddagger$ & no \\
        \cref{clm:girth6-28}  & $4{,}624{,}501$ & 3 days (measured) & yes \\
        \midrule
        remaining Claims 3--4 & & unknown & no \\
        \bottomrule
    \end{tabular}
    \par\smallskip
    {\footnotesize $^\ast$Projected number of distinct Claim 3 cores.
    $^\dagger N_4$ is the as-yet unknown number of globally merged Claim 4
    groups, in millions.  $^\ddagger$Measured 25--30-fold effective speedup,
    excluding preparation; groups with more markers may cost more.\par}
\end{table}
