...
\begin{proof}[Proposed joint computational proof]
    ...
    simple.  It seeks one of the following certificates.

    First, if the replacement path lies on a second 5-cycle, the preparation
    ...
    Otherwise, the preparation looks for a clean 5-cycle $D$ of $H$ that
    contains $b$, and contracts it to a degree-five vertex.  Let $J$ be one
    selected canonical contraction.  If the code of $J$ is among the
    retained Claim~3 cores, its drawing with all five attachments uncrossed
    is already one of the requirements checked in
    \cref{clm:flexible26}.  Every other selected code is checked once for
    precisely this all-five-attachments requirement.  Part~(i) of
    \cref{lem:5-cycle-star-expansion} expands $J$ to a drawing of $H$ in
    which the star at $b$ is uncrossed, and
    \cref{lem:5-cycle-expansion} then expands $H$ to $G$.

    If neither lower-order certificate is available, one canonical
    reduction $(H,b)$ is retained.  The computation checks directly that
    $H$ has a 1-planar drawing in which the three edges incident with $b$ are
    uncrossed; \cref{lem:5-cycle-expansion} then restores $G$.  Thus marked
    order-26 reductions are used only for the residual class, and no
    flexibility assumption beyond the ordinary Claim~3 statement is needed.
    The global
    preparation and verification described in
...
was \(0.05887\)--\(0.07693\).  Thus the girth-five part of Claim~3 projects
to 19.7--23.6 hours at the measured 30--25-fold effective speedup, with a
17.1--26.8 hour sampling envelope.

A server test of the joint Claim~4 preparation with direct order-28
residuals processed
$5{,}465{,}734$ parents from three complete Minibaum parts.  It classified
$3{,}026{,}393$ as \texttt{D}, $2{,}381{,}534$ as \texttt{C}, and
$57{,}807$ as \texttt{R}.  The \texttt{R} records were all distinct, and a
uniformly spaced 100-record sample took 0.930 seconds per graph.  Scaling
only preparation and the \texttt{R} class to the full connected family gives
1,941 CPU-hours, or 64.7--77.6 wall-clock hours at 30--25-fold effective
speedup.  This already exceeds the three-day target at 25-fold speedup and
assigns zero cost to every \texttt{C} record.  Adding Claim~3 gives a joint
floor of 84.9--101.9 hours, still without Claim~4 core verification or graph
generation.  Thus direct verification of the residual parents was rejected.

In the revised fallback, each residual parent retains one marked order-26
reduction instead.  On 85 residuals from a uniformly spaced
100,000-parent sample, the marked reductions were all distinct and all
passed in 22.15 seconds, or 0.261 seconds per record.  Removing crossing
candidates incident with the three prescribed edges before encoding reduced
the matched time to 14.73 seconds, or 0.173 seconds per record.  At the
server's measured residual fraction, this lowers the residual term from
1,782 to approximately 332 CPU-hours.  Together with Claim~3 and Claim~4
preparation, the resulting floor is 36.6--43.9 hours at 30--25-fold speedup.
This is
promising but is not yet a three-day bound: the server residual sample and
the global Claim~4 core set remain to be measured.
...
\begin{table}[!ht]
    ...
    \begin{tabular}{@{}lrrc@{}}
        ...
        \cref{clm:flexible26} & $4.54$ million${}^\ast$ & 20--27 hours (projected) & no \\
        \cref{clm:girth5-28}  & joint with Claim 3 & see below & no \\
        \cref{clm:girth6-28}  & $4{,}624{,}501$ & 3 days (measured) & yes \\
        \midrule
        remaining Claims 3--4 & & at least 37--44 hours${}^\dagger$ & no \\
        \bottomrule
    \end{tabular}
    \par\smallskip
    {\footnotesize $^\ast$Projected number of distinct Claim 3 cores.
    $^\dagger$Floor for the revised residual fallback: Claim 4 core
    verification and graph generation are assigned zero cost.\par}
\end{table}
